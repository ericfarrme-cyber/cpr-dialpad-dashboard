...
function StoreDashboard() {
  ...
  var handleImport = async function(e) {
    ...
    try {
      ...
      var allRows=[];
      wb.SheetNames.forEach(function(sn){var ws=wb.Sheets[sn];var data=XLSX.utils.sheet_to_json(ws,{header:1,defval:"",raw:false});if(data.length<2)return;var hi=-1;var headerIndicators=["customer name","date of appt","date set","phone number","appt reason","scheduled by","did they arrive","time of appt","quotes"];for(var ri=0;ri<Math.min(data.length,10);ri++){var row=data[ri];if(!row)continue;var matchCount=0;for(var ci=0;ci<row.length;ci++){var cv=String(row[ci]||"").toLowerCase().trim();for(var ii=0;ii<headerIndicators.length;ii++){if(cv.includes(headerIndicators[ii])){matchCount++;break;}}}if(matchCount>=2){hi=ri;break;}}if(hi<0)return;var col={};var hdr=data[hi];for(var ci=0;ci<hdr.length;ci++){var h=String(hdr[ci]||"").toLowerCase().trim();if(h.includes("customer name"))col.name=ci;else if(h.includes("phone"))col.phone=ci;else if(h.includes("date set"))col.date_set=ci;else if(h.includes("date of"))col.date_appt=ci;else if((h.includes("time")&&!h.includes("date"))||h.includes("time of appt"))col.time=ci;else if(h.includes("reason")||h.includes("quotes"))col.reason=ci;else if(h.includes("scheduled")||h.includes("your name"))col.scheduled_by=ci;else if(h.includes("arrive"))col.arrived=ci;}if(col.name===undefined){col.name=0;}if(col.scheduled_by===undefined&&col.reason!==undefined){var nxr=col.reason+1;if(nxr<hdr.length)col.scheduled_by=nxr;}if(col.arrived===undefined&&col.scheduled_by!==undefined){var nxs=col.scheduled_by+1;if(nxs<hdr.length)col.arrived=nxs;}for(var ci=0;ci<hdr.length;ci++){var h=String(hdr[ci]||"").toLowerCase().trim();if(h==="notes"&&ci!==col.reason)col.notes=ci;}if(col.notes===undefined&&col.arrived!==undefined){var nxa=col.arrived+1;if(nxa<hdr.length)col.notes=nxa;}
