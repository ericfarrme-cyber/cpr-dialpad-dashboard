...
//            elapsedMs, rawRows } }
// ── LIVE TODAY-LEG: synchronous call-list API (GET /call) ──────────────────
// Dialpad's is_today EXPORTS stopped completing (queue-side, 2026-06-09), but the
// live call log is current — verified against the Dialpad UI during the outage.
// These helpers fetch today's calls directly and map them into the same row shape
// the export CSV produces, so saveCallRecords/daily_call_volume need no changes.
//
// CLASSIFICATION RULE (feeds is_answered/is_missed → answer-rate bonus):
//   date_connected present  → answered (a human connected; probe sample confirmed:
//                             connected call had duration 313127ms, others 0)
//   no date_connected       → inbound: missed/unanswered; outbound: not connected
// availability is left "" intraday — next morning's historical export re-upserts
// the same call_ids with Dialpad's authoritative categories + availability.

function msSinceLocalMidnight(now) {
  // DST-proof: parse the current local (Indianapolis) wall-clock time.
...
function deriveLiveCategories(item) {
  var connected = !!item.date_connected;
  if ((item.direction || "") === "outbound") {
    return connected ? "outbound,outbound_connected,user_initiated" : "outbound";
  }
  return connected ? "answered,human_agent,inbound" : "inbound,missed,unanswered";
}

function mapLiveCall(item) {
  var tgt = item.target || {};
  // Department-level row (the one the dashboard counts): target.type says so;
  // fallback — the entry-point call's id equals its own call_id.
  var isDept = tgt.type === "department" ||
    (item.entry_point_call_id != null && String(item.entry_point_call_id) === String(item.call_id));
  var started = Number(item.date_started);
  var rang = Number(item.date_rang);
  var connected = Number(item.date_connected);
  var ended = Number(item.date_ended);
  var ringingMs = null;
  if (!isNaN(rang)) {
    if (!isNaN(connected)) ringingMs = connected - rang;
    else if (!isNaN(ended)) ringingMs = ended - rang;
  }
  return {
    call_id: String(item.call_id || ""),
    date_started: isNaN(started) ? "" : new Date(started).toISOString(),
    name: tgt.name || "",
    direction: item.direction || "",
    categories: deriveLiveCategories(item),
    target_type: isDept ? "department" : (tgt.type || ""),
    external_number: item.external_number || "",
    availability: "",
    was_recorded: (item.was_recorded === true || (Array.isArray(item.call_recording_ids) && item.call_recording_ids.length > 0)) ? "True" : "False",
    ringing_duration: ringingMs != null && ringingMs >= 0 ? String(ringingMs / 1000) : "",
    talk_duration: item.duration ? String(Number(item.duration) / 1000) : "",
  };
}

export async function fetchLiveTodayCalls(deptIds) {
  var startedAfter = Date.now() - msSinceLocalMidnight(new Date());
  var rows = [];
  var errors = [];
  for (var d = 0; d < deptIds.length; d++) {
    var deptId = deptIds[d];
    var cursor = null;
    try {
      for (var page = 0; page < 10; page++) {
        var u = DIALPAD_BASE + "/call?target_id=" + encodeURIComponent(deptId) +
          "&target_type=department&started_after=" + startedAfter + "&limit=50" +
        ...
        if (!res.ok) throw new Error("call-list HTTP " + res.status);
        var json = await res.json();
        var items = json.items || [];
        items.forEach(function (it) { rows.push(mapLiveCall(it)); });
        cursor = json.cursor || null;
        if (!cursor || items.length === 0) break;
      }
    } catch (e) {
      console.error("[dialpad-stats] live call-list error for dept " + deptId + ": " + e.message);
      errors.push({ deptId: deptId, error: e.message });
    }
  }
...
export async function fetchStoreCalls(storeKey, opts) {
  ...
  deptIds.forEach(function (deptId) {
    ...
    // NOTE: is_today exports are intentionally NOT initiated anymore. They stopped
    // completing within any collectible window on 2026-06-09 (measured: 40+ min of
    // 10-minute-interval polling, then "Results have expired"). Today's calls now
    // come from the synchronous call-list API below — see fetchLiveTodayCalls.
  });
  ...
  // ── Live today-leg: synchronous, no queue, no expiry ──
  var live = await fetchLiveTodayCalls(deptIds);
  var liveAdded = 0;
  ...
  if (liveAdded > 0) {
    console.log("[dialpad-stats] " + storeKey + ": live call-list added " + liveAdded + " row(s) for today");
  }
  ...
  return {
    ...
    meta: {
      ...
      liveTodayRows: liveAdded,
      liveTodayErrors: live.errors,
