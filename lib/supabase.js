...
export async function saveCallRecords(records) {
  ...
  const rows = records.map(r => {
    ...
    return {
      ...
      is_missed: !cats.includes("answered") && (
        (cats.includes("missed") && !cats.includes("dismissed")) ||
        (cats.includes("unanswered") && !cats.includes("transferred"))
      ),
...
export async function getHourlyMissed(daysBack = 30) {
  ...
  // Post-filter: exclude dismissed and unanswered_transferred
  return (data || []).filter(function(r) {
    var cats = (r.categories || "").toLowerCase();
    if (cats.includes("answered")) return false;
    if (cats.includes("dismissed")) return false;
    if (cats.includes("unanswered") && cats.includes("transferred")) return false;
    return true;
  });
}
