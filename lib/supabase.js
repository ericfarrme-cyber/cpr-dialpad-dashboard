...
export async function saveAuditResult(audit) {
  ...
  const row = {
    ...
    // Qualitative grading dimensions (added with cx insights migration).
    // These are observation-only — they do NOT feed into existing scoring math.
    // null is the correct value when call_type is "non_scorable" or AI didn't return them.
    tone_score: audit.tone_score != null ? audit.tone_score : null,
    clarity_score: audit.clarity_score != null ? audit.clarity_score : null,
    empathy_score: audit.empathy_score != null ? audit.empathy_score : null,
    qualitative_notes: audit.qualitative_notes || null,
  };
...
export async function getEmployeeStatsFromAudits(store) {
  ...
  // Paginate through ALL matching rows. The Supabase JS client goes through
  // PostgREST, which caps every response at the project "Max rows" setting
  // regardless of any .limit(). Looping with .range() in pageSize chunks pulls
  // the complete set no matter how large the table grows — nothing is trimmed
  // and the result no longer depends on the Max rows value. A stable .order()
  // is required so rows don't shift between pages.
  // NOTE: keep pageSize <= the project's Max rows setting.
  const pageSize = 1000;
  const buildQuery = () => {
    let q = supabase
      .from("audit_results")
  ...
  while (true) {
    ...
    for (let i = 0; i < page.length; i++) data.push(page[i]);
    if (page.length < pageSize) break; // last (partial) page reached
    from += pageSize;
  }
...
export async function saveCallRecords(records) {
  ...
  const rows = records.map(r => {
    ...
    return {
      ...
      is_answered: catParts.indexOf("answered") >= 0,
      is_missed: catParts.indexOf("answered") < 0 && (catParts.indexOf("missed") >= 0 || catParts.indexOf("unanswered") >= 0) && catParts.indexOf("dismissed") < 0,
...
export async function getHourlyMissed(daysBack = 30) {
  if (!supabase) return [];
  const { data, error } = await supabase.from("call_records").select("date_started, store, categories, availability")
    .eq("target_type", "department").eq("direction", "inbound").eq("is_missed", true)
    .gte("date_started", new Date(Date.now() - daysBack * 86400000).toISOString());
  if (error) console.error("Hourly missed error:", error);
  // Post-filter: exclude dismissed and transferred calls that were actually answered
  return (data || []).filter(function(r) {
    var cats = (r.categories || "").toLowerCase();
    var parts = cats.split(/[,\s|]+/);
    // If "answered" is a standalone category (not inside "unanswered"), exclude
    if (parts.indexOf("answered") >= 0) return false;
    // If "dismissed" is present, someone declined the transfer but call was handled
    if (parts.indexOf("dismissed") >= 0) return false;
    return true;
  });
}
...
export async function getDOWMissed(daysBack = 30) {
  if (!supabase) return [];
  const { data, error } = await supabase.from("call_records").select("date_started, store, categories")
    .eq("target_type", "department").eq("direction", "inbound").eq("is_missed", true)
    .gte("date_started", new Date(Date.now() - daysBack * 86400000).toISOString());
  if (error) console.error("DOW missed error:", error);
  return (data || []).filter(function(r) {
    var parts = (r.categories || "").toLowerCase().split(/[,\s|]+/);
    if (parts.indexOf("answered") >= 0) return false;
    if (parts.indexOf("dismissed") >= 0) return false;
    return true;
  });
...
export async function getCallbackData(daysBack = 30) {
  ...
  const { data: missed, error: e1 } = await supabase.from("call_records").select("call_id, date_started, store, external_number, categories")
    .eq("target_type", "department").eq("direction", "inbound").eq("is_missed", true)
  ...
  // Post-filter missed to exclude dismissed/transferred
  var filteredMissed = (missed || []).filter(function(r) {
    var parts = (r.categories || "").toLowerCase().split(/[,\s|]+/);
    if (parts.indexOf("answered") >= 0) return false;
    if (parts.indexOf("dismissed") >= 0) return false;
    return true;
  });
  return { missed: filteredMissed, outbound: outbound || [] };
}

export async function getVoicemailData(daysBack = 30) {
  if (!supabase) return { voicemails: [], outbound: [] };
  const since = new Date(Date.now() - daysBack * 86400000).toISOString();
  const { data: vms, error: e1 } = await supabase.from("call_records")
    .select("call_id, date_started, store, store_name, external_number, talk_duration, ringing_duration")
    .eq("target_type", "department").eq("direction", "inbound").eq("is_voicemail", true)
    .gte("date_started", since)
    .order("date_started", { ascending: false });
  const { data: outbound, error: e2 } = await supabase.from("call_records")
    .select("date_started, store, external_number, talk_duration")
    .eq("direction", "outbound").gte("date_started", since)
    .order("date_started", { ascending: true });
  if (e1) console.error("Voicemail query error:", e1);
  if (e2) console.error("Voicemail outbound error:", e2);
  return { voicemails: vms || [], outbound: outbound || [] };
}
...
// Days a store closed early or entirely (holidays, weather, staffing). Missed
// calls outside the hours actually kept are not a performance failure — see
// lib/store-closures.js.
export async function getStoreClosures(daysBack = 30) {
  if (!supabase) return [];
  ...
  const { data, error } = await supabase
    ...
    .gte("closure_date", since.toISOString().split("T")[0]);
  if (error) throw new Error("store_closures: " + error.message);
  return data || [];
}
