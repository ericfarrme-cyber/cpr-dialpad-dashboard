...
export function transformToDailyCalls(rows) {
  const dr = rows.filter(isDept); const m = {};
  dr.forEach(row => { const sk = getStore(row); if (!sk) return; const d = new Date(row.date_started); if (isNaN(d)) return;
    const dk = `${d.getMonth()+1}/${d.getDate()}`;
    if (!m[dk]) { m[dk] = { date: dk }; STORE_KEYS.forEach(k => { m[dk][`${k}_total`] = 0; m[dk][`${k}_answered`] = 0; }); }
    m[dk][`${sk}_total`]++; if (isAnswered(row)) m[dk][`${sk}_answered`]++;
  });
...
export function transformToHourlyMissed(rows) {
  const dr = rows.filter(isDept); const hm = {};
...
export function transformToDOWMissed(rows) {
  const dr = rows.filter(isDept); const days=["Sun","Mon","Tue","Wed","Thu","Fri","Sat"];
...
export const SAMPLE_DAILY_CALLS = Array.from({length:30},(_,i)=>{const d=new Date(2026,1,9+i);const w=d.getDay();const we=w===0||w===6;const b=we?18:42;const s=((i*13+7)%10);return{date:`${d.getMonth()+1}/${d.getDate()}`,fishers_total:b+s,fishers_answered:b-4+Math.min(s,8),bloomington_total:b-3+s,bloomington_answered:b-7+Math.min(s,7),indianapolis_total:b+2+s,indianapolis_answered:b-2+Math.min(s,9)};});
