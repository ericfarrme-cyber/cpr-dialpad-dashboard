...
function getStore(row) { return row._storeKey || row.store || null; }
function isAnswered(row) {
  // Live data: categories string; Stored data: is_answered boolean
  if (row.is_answered === true || row.is_answered === "true") return true;
  if (row.is_answered === false || row.is_answered === "false") return false;
  return (row.categories || "").includes("answered");
}
function isMissed(row) {
  // Live data: categories string; Stored data: is_missed/is_voicemail boolean
  if (row.is_missed === true || row.is_missed === "true") return true;
  if (row.is_voicemail === true || row.is_voicemail === "true") return true;
  var c = row.categories || "";
  return c.includes("missed") || c.includes("unanswered") || c.includes("abandoned") || c.includes("voicemail");
}
