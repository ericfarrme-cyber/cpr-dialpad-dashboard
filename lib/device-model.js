...
function matchIphone(s) {
  ...
  var m = s.match(/\biphone\s*(\d{1,2}\s*e|\d{1,2}|se|xr|xs|x)\b\s*(pro\s*max|pro|plus|mini|max)?/);
  if (!m) return null;
  ...
  var vmap = { "pro max": " Pro Max", "pro": " Pro", "plus": " Plus", "mini": " Mini", "max": " Max" };
...
function matchConsole(s) {
  if (/\bps\s*5\b|\bplaystation\s*5\b/.test(s)) return /controller/.test(s) ? "PS5 Controller" : "PlayStation 5";
  if (/\bps\s*4\b|\bplaystation\s*4\b/.test(s)) return /controller/.test(s) ? "PS4 Controller" : "PlayStation 4";
  if (/\bps\s*3\b|\bplaystation\s*3\b/.test(s)) return "PlayStation 3";
