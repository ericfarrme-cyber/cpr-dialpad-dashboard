...
function matchIphone(s) {
  ...
  // "SE3", "SE 3", "SE (3rd gen)", "SE 2020" are all iPhone SE: the sheet keeps
  // one SE line and the register rarely says which generation.
  var m = s.match(/\biphone\s*(se\s*(?:\(?\s*\d(?:st|nd|rd|th)?\s*gen(?:eration)?\s*\)?|\d{1,4})?|\d{1,2}\s*e|\d{1,2}|xr|xs|x)\b\s*(pro\s*max|pro|plus|mini|max|air)?/);
  if (!m) return null;
  var num = m[1].replace(/\s+/g, "").toUpperCase();
  var variant = (m[2] || "").replace(/\s+/g, " ").trim();
  if (num.indexOf("SE") === 0) return "iPhone SE";
...
// Windows PCs. RepairQ names them by brand, not model ("Dell Dell Laptop -
// Any / Other", "HP HP Laptop - Any / Other"), and callers say "my Dell
// laptop" — so the brand + type IS the model here. Matt saw Price & Demand
// list MacBooks and no Windows machines at all: every one of these was
// filed as a bare "laptop" category. A brand on its own ("HP") stays generic.
var PC_BRANDS = { dell: "Dell", hp: "HP", lenovo: "Lenovo", asus: "ASUS", acer: "Acer", msi: "MSI", toshiba: "Toshiba", samsung: "Samsung" };
function matchPc(s) {
  if (/\bsurface\b/.test(s)) return "Microsoft Surface";
  if (/\bchromebook\b/.test(s)) return "Chromebook";
  var b = s.match(/\b(dell|hp|lenovo|asus|acer|msi|toshiba|samsung)\b/);
  if (!b) {
    // Product lines that name the brand without saying it.
    if (/\b(thinkpad|ideapad|thinkcentre)\b/.test(s)) b = [null, "lenovo"];
    else if (/\b(inspiron|latitude|xps|alienware|optiplex)\b/.test(s)) b = [null, "dell"];
    else if (/\b(pavilion|envy|spectre|elitebook|probook|omen)\b/.test(s)) b = [null, "hp"];
    else if (/\b(vivobook|zenbook|rog)\b/.test(s)) b = [null, "asus"];
    else if (/\b(aspire|nitro|predator|swift)\b/.test(s)) b = [null, "acer"];
    else return null;
  }
  if (/\b(desktop|all[\s-]*in[\s-]*one|aio|tower|optiplex|thinkcentre)\b/.test(s)) return PC_BRANDS[b[1]] + " Desktop";
  if (/\b(laptop|notebook|thinkpad|ideapad|inspiron|latitude|xps|alienware|pavilion|envy|spectre|elitebook|probook|omen|vivobook|zenbook|rog|aspire|nitro|predator|swift|yoga)\b/.test(s)) return PC_BRANDS[b[1]] + " Laptop";
  return null;
}
...
function familyOf(canonical) {
  ...
  if (/^MacBook|^iMac|Laptop$|Desktop$|^Microsoft Surface|^Chromebook/.test(canonical)) return "computer";
...
function resolveOne(phrase) {
  ...
  // Brand + type has to be read BEFORE brands are stripped — stripping turns
  // "Dell Laptop" into a bare "laptop".
  var pc = matchPc(s);
  if (pc) return { canonical: pc, family: familyOf(pc), specified: true, reason: null };
