...
export function preAuditFilter(call) {
  ...
  // 2. Very short calls — likely hangups or misdials
  // Dialpad talk_duration is in MINUTES (e.g. 1.49 = ~1 min 30s)
  var talkDuration = call.talk_duration ? parseFloat(call.talk_duration) : null;
  if (talkDuration !== null && talkDuration < 0.25) {
    return { pass: false, reason: "too_short", detail: "Talk duration " + (talkDuration * 60).toFixed(0) + "s (< 15s minimum)" };
  }
...
═══ STEP 3.5 — QUALITATIVE GRADING ═══

If call_type is "non_scorable", SKIP this step (set qualitative fields to null).
Otherwise, evaluate the EMPLOYEE's interpersonal handling on three independent dimensions.

These scores DO NOT affect the structural score above — they're a separate signal used for coaching.
Use the FULL 1-5 range. A 3 is a genuinely neutral baseline (professional but unremarkable), NOT a "default safe answer."
Do not anchor toward 3 to avoid taking a position. If the employee was warm, give a 4 or 5. If they were curt, give a 2.

tone_score (1-5): How warm and genuinely engaged did the employee sound?
  5 = warm, friendly, natural; sounded like they actually cared about the customer
  4 = courteous and pleasant throughout
  3 = neutral / professional but flat
  2 = curt or rushed; transactional only; minimal warmth
  1 = cold, dismissive, annoyed, or hostile

clarity_score (1-5): How easy was the employee to understand if you were a typical customer (non-technical)?
  5 = explained things in plain language; checked for understanding; no unexplained jargon
  4 = clear and confident; minimal jargon
  3 = adequate but assumed customer would follow without checking
  2 = some confusion or technical jargon used without explanation
  1 = confusing, unhelpful, or contradictory

empathy_score (1-5): How well did the employee respond to the customer's situation or emotion?
  5 = acknowledged customer's frustration/urgency; reassuring; problem-solving mindset
  4 = listened and addressed concerns directly
  3 = answered the question but didn't engage with feelings
  2 = ignored emotional cues; pushed agenda over customer's situation
  1 = condescending or unhelpful in the face of an upset customer

qualitative_notes (string, ~25 words max): One sentence describing what stood out about the employee's manner. This is for coaching context, so be specific. Examples:
  - "Warm and patient throughout, used customer's name twice."
  - "Used 'GPU' and 'BGA' without explanation; customer sounded confused."
  - "Skipped over caller's frustration about the second visit."
  - "Took time to confirm understanding before offering to schedule."
If genuinely unremarkable, say so: "Polite and professional but unremarkable."
...
Respond ONLY with valid JSON — no markdown, no explanation, no preamble:
{
  ...
  "score": 0.00,
  "max_score": 4.01,
  "tone_score": 1-5 integer (or null if non_scorable),
  "clarity_score": 1-5 integer (or null if non_scorable),
  "empathy_score": 1-5 integer (or null if non_scorable),
  "qualitative_notes": "string under 30 words (or null if non_scorable)"
}
