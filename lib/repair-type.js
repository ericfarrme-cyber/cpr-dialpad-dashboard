...
// Which quality tier a RepairQ catalog line is. The catalog encodes it in the
// name: "(OLED)", "(LCD / Other)", and "Original Apple Part" vs "Non Original"
// for OEM vs aftermarket. Shared by the Price Book and Price & Demand so a
// screen is tiered the same way everywhere.
export function tierOfCatalogLine(name) {
...
export var TIER_ORDER = ["OEM", "OLED", "LCD", "Digitizer", "Untiered"];

// "(Protected)" lines are insurance claims — the insurer sets the price per
// claim, so they are neither a list price nor a discount. Every pricing read
// must skip them or insurance looks like heavy discounting.
export function isInsuranceLine(name) {
  return /\(protected\)/i.test(String(name || ""));
}
