...
**Last updated:** 2026-09-10 (09-10 meeting with Matt; P&L manual entry, monthly trend, Duncan's tab, daily log sync, light mode, My Performance call attribution — § 11)
...
10a. **Zero / negative profit ticket report.** Matt: *"a section that we can put like tickets that are zero or negative profit... sort by month or by employee."* Live count: **272 tickets at or below $0 — 100 exactly zero, 172 negative, totalling −$6,128.37.** Put it inside Ticket Compliance, not a new tab (Eric wants fewer tabs, more per tab). This is how Luke's refunded benches and mis-logged tickets surface.
10b. **Ticket views show former staff and non-humans.** `ticket_grades.employee_added` holds 22 distinct names, 13 off the active roster across 837 tickets: Mahmoud Awad, Jordan Ross, Matthew Ziegler, Joseph Ciceu, plus **"Assurant ServiceNetwork" (505 tickets)** and "API". The roster API already filters `active = true`; the ticket views do not. Also **"Sam Tomey" does not match roster "Samuel Tomey"** (58 added / 48 repaired) — same class of bug as the audit aliases, now fixable with `lib/roster-resolver.js`.
10c. **Price sensitivity + call-to-repair conversion.** The largest new build. Per model: list price, average discount, actual average sale price, margin, and full-price vs discounted counts. Cross-referenced with call data: opportunity calls per model and conversion to repairs (Matt: *"out of the 200 iPhone 17 calls across three locations, you got 15 repairs"*). Flag opportunity calls that did not convert **and** had no appointment offered. Add a "what customers want" breakdown to drive reorder quantities. Same treatment for consoles. **`demand-analysis` is staffing/hourly coverage only — none of this exists.** Context: OLED prices collapsed (17 Pro $50, 14 Pro Max $39, was $60–70), margins now $120+ per iPhone repair vs $80–100, so the price/volume question is live and unanswered.
