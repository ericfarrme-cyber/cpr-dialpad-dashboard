...
var ROLES = [
  { id: "admin", label: "Admin", color: "var(--pink)", desc: "Full access — manage users, all data, system settings" },
  { id: "manager", label: "Manager", color: "var(--purple)", desc: "Full dashboard access — audit, grade, view all employees" },
  { id: "employee", label: "Employee", color: "var(--cyan)", desc: "Limited view — own scores, schedule, store stats only" },
];
  ...
  if (!auth || auth.role !== "admin") {
    return (
      <div style={{ padding:40,textAlign:"center" }}>
        ...
        <div style={{ color:"var(--text-primary)",fontSize:16,fontWeight:700 }}>Admin Access Required</div>
        <div style={{ color:"var(--text-muted)",fontSize:13,marginTop:8 }}>Only administrators can manage dashboard users.</div>
      </div>
    );
  }

  if (loading) return <div style={{ padding:40,textAlign:"center",color:"var(--text-muted)" }}>Loading users...</div>;
  ...
  return (
    <div>
      ...
      <div style={{ display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:14,marginBottom:24 }}>
        <StatCard label="Total Active Users" value={activeUsers.length} accent="var(--purple)" />
        <StatCard label="Admins" value={adminCount} accent="var(--pink)" />
        <StatCard label="Managers" value={managerCount} accent="var(--purple)" />
        <StatCard label="Employees" value={empCount} accent="var(--cyan)" />
      </div>
      ...
      <div style={{ background: "linear-gradient(135deg, #7B2FFF12, #FF2D9508)", borderRadius: 14, padding: 24, marginBottom: 28, border: "1px solid #7B2FFF44" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 16, flexWrap: "wrap", gap: 12 }}>
          <div>
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              ...
              <span style={{ color: "var(--text-primary)", fontSize: 18, fontWeight: 800 }}>Celebration Queue</span>
            </div>
            <div style={{ color: "var(--text-secondary)", fontSize: 12, marginTop: 4 }}>Tier-ups, streak bonuses earned, and Diamond plaque qualifiers — announce them, pay them, track them.</div>
          </div>
        ...
        <div style={{ display: "flex", gap: 12, marginBottom: 16, flexWrap: "wrap" }}>
          <div>
            <div style={{ color: "var(--text-secondary)", fontSize: 10, textTransform: "uppercase", marginBottom: 4, fontWeight: 700 }}>Store</div>
            <div style={{ display: "flex", gap: 4, background: "var(--bg-card)", borderRadius: 8, padding: 4 }}>
              {[{ v: "all", l: "All Stores" }, { v: "fishers", l: "Fishers" }, { v: "bloomington", l: "Bloomington" }, { v: "indianapolis", l: "Indianapolis" }].map(function(opt) {
                return (
                  <button key={opt.v} onClick={function(){setCelebStoreFilter(opt.v);}} style={{
                    ...
                    background: celebStoreFilter === opt.v ? "var(--purple)" : "transparent",
                    color: celebStoreFilter === opt.v ? "#FFF" : "var(--text-secondary)",
                  }}>{opt.l}</button>
                );
              })}
            </div>
          ...
          <div>
            <div style={{ color: "var(--text-secondary)", fontSize: 10, textTransform: "uppercase", marginBottom: 4, fontWeight: 700 }}>Status</div>
            <div style={{ display: "flex", gap: 4, background: "var(--bg-card)", borderRadius: 8, padding: 4 }}>
              {[{ v: "pending", l: "Pending" }, { v: "all", l: "All" }].map(function(opt) {
                return (
                  <button key={opt.v} onClick={function(){setCelebStatusFilter(opt.v);}} style={{
                    padding: "5px 12px", borderRadius: 6, border: "none", cursor: "pointer", fontSize: 11, fontWeight: 700,
                    background: celebStatusFilter === opt.v ? "var(--pink)" : "transparent",
                    color: celebStatusFilter === opt.v ? "#FFF" : "var(--text-secondary)",
                  }}>{opt.l}</button>
                );
              })}
            </div>
        ...
        {/* Events list */}
        {celebLoading ? (
          <div style={{ padding: 30, textAlign: "center", color: "var(--text-muted)", fontSize: 13 }}>Loading events...</div>
        ) : celebrations.length === 0 ? (
          <div style={{ padding: 30, textAlign: "center", color: "var(--text-muted)", fontSize: 13, background: "var(--bg-card)", borderRadius: 10 }}>
          ...
          <div style={{ display: "grid", gap: 10 }}>
            {celebrations.map(function(ev) {
              var typeMeta = {
                tier_up: { icon: "\uD83D\uDE80", color: "var(--cyan)", label: "Tier Up", desc: "Promoted from " + (ev.prior_tier || "?") + " to " + (ev.new_tier || "?") },
                ...
                diamond_plaque: { icon: "\u2B50", color: "var(--cyan)", label: "Diamond Plaque", desc: ev.streak_length + " Diamond months in " + ev.event_period },
              }[ev.event_type] || { icon: "\uD83C\uDF1F", color: "var(--purple)", label: ev.event_type, desc: "" };
              ...
              return (
                <div key={ev.id} style={{
                  background: "var(--bg-card-inner)", borderRadius: 12, padding: "14px 18px",
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 14, flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ minWidth: 0, flex: 1 }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
                        <span style={{ color: "var(--text-primary)", fontSize: 15, fontWeight: 800 }}>{ev.employee_name}</span>
                        ...
                        <span style={{ color: "var(--text-muted)", fontSize: 11 }}>{(ev.store || "").toUpperCase()} &middot; {ev.event_period}</span>
                      </div>
                      <div style={{ color: "var(--text-secondary)", fontSize: 12, marginTop: 4 }}>{typeMeta.desc}</div>
                      {bonusLabel && (
                        <div style={{ marginTop: 6, display: "inline-flex", alignItems: "center", gap: 6, padding: "3px 10px", background: "#FBBF2418", border: "1px solid #FBBF2444", borderRadius: 6 }}>
                          <span style={{ color: "var(--yellow)", fontSize: 12, fontWeight: 800 }}>Bonus: {bonusLabel}</span>
                        </div>
                      )}
                    </div>
                  </div>
                  <div style={{ display: "flex", gap: 6, flexWrap: "wrap", justifyContent: "flex-end" }}>
                    {!isAnnounced && !isDismissed && (
                      <button onClick={function(){celebAction(ev.id, "mark_announced");}} style={{
                        ...
                        background: "#4ADE8018", color: "var(--green)", fontSize: 11, fontWeight: 700, cursor: "pointer",
                      }}>{"\uD83D\uDCE3"} Announced</button>
                    )}
                    {isAnnounced && !isDismissed && (
                      <span style={{ padding: "7px 10px", borderRadius: 6, background: "#4ADE8018", color: "var(--green)", fontSize: 10, fontWeight: 700 }}>
                        {"\u2705"} Announced
                      </span>
                    )}
                    {bonusActionable && !isPaid && !isDismissed && (
                      <button onClick={function(){celebAction(ev.id, "mark_paid");}} style={{
                        ...
                        background: "#FBBF2418", color: "var(--yellow)", fontSize: 11, fontWeight: 700, cursor: "pointer",
                      }}>{"\uD83D\uDCB0"} Mark Paid</button>
                    )}
                    {bonusActionable && isPaid && !isDismissed && (
                      <span style={{ padding: "7px 10px", borderRadius: 6, background: "#FBBF2418", color: "var(--yellow)", fontSize: 10, fontWeight: 700 }}>
                        {"\u2705"} Paid
                      </span>
                    )}
                    {!isDismissed && (
                    ...
                    {isDismissed && (
                      <span style={{ padding: "7px 10px", borderRadius: 6, color: "var(--text-muted)", fontSize: 10, fontStyle: "italic" }}>Dismissed</span>
                    )}
                  </div>
                </div>
              );
      ...
      <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16 }}>
        <div style={{ color:"var(--text-primary)",fontSize:16,fontWeight:700 }}>Dashboard Users</div>
      ...
      {actionMsg && (
        <div onClick={function(){setActionMsg(null);}}
          style={{ padding:"10px 16px",borderRadius:8,marginBottom:16,cursor:"pointer",background:actionMsg.type==="success"?"#4ADE8012":"#F8717112",border:"1px solid "+(actionMsg.type==="success"?"#4ADE8033":"#F8717133"),color:actionMsg.type==="success"?"var(--green)":"var(--red)",fontSize:13 }}>
          {actionMsg.text}
        </div>
      )}
      ...
      {/* Add user form */}
      {showAdd && (
        <div style={{ background:"var(--bg-card)",borderRadius:12,padding:24,marginBottom:20,border:"1px solid #7B2FFF33" }}>
          <div style={{ color:"var(--text-primary)",fontSize:14,fontWeight:700,marginBottom:16 }}>Add New User</div>
          <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:16 }}>
            <div>
              <label style={{ color:"var(--text-secondary)",fontSize:11,display:"block",marginBottom:4 }}>Email *</label>
              <input type="email" value={form.email} onChange={function(e){setForm(Object.assign({},form,{email:e.target.value}));}}
                placeholder="user@gmail.com"
                style={{ width:"100%",padding:"10px 12px",borderRadius:8,border:"1px solid var(--border)",background:"var(--bg-card-inner)",color:"var(--text-primary)",fontSize:13,outline:"none",boxSizing:"border-box" }} />
            </div>
            <div>
              <label style={{ color:"var(--text-secondary)",fontSize:11,display:"block",marginBottom:4 }}>Full Name</label>
              <input type="text" value={form.name} onChange={function(e){setForm(Object.assign({},form,{name:e.target.value}));}}
                placeholder="John Smith"
                style={{ width:"100%",padding:"10px 12px",borderRadius:8,border:"1px solid var(--border)",background:"var(--bg-card-inner)",color:"var(--text-primary)",fontSize:13,outline:"none",boxSizing:"border-box" }} />
            </div>
            <div>
              <label style={{ color:"var(--text-secondary)",fontSize:11,display:"block",marginBottom:4 }}>Role *</label>
            ...
            <div>
              <label style={{ color:"var(--text-secondary)",fontSize:11,display:"block",marginBottom:4 }}>Primary Store</label>
            ...
            {ROLES.map(function(r) {
              ...
              return (
                <div key={r.id} onClick={function(){setForm(Object.assign({},form,{role:r.id}));}}
                  style={{ padding:12,borderRadius:8,border:"1px solid "+(isSelected?r.color+"55":"var(--border)"),background:isSelected?r.color+"08":"var(--bg-card-inner)",cursor:"pointer" }}>
                  <div style={{ color:r.color,fontSize:12,fontWeight:700,marginBottom:4 }}>{r.label}</div>
                  <div style={{ color:"var(--text-muted)",fontSize:10,lineHeight:1.3 }}>{r.desc}</div>
                </div>
              );
            })}
          </div>
          ...
          <button onClick={handleAdd} disabled={saving}
            style={{ padding:"10px 24px",borderRadius:8,border:"none",background:saving?"var(--text-muted)":"var(--purple)",color:"#FFF",fontSize:13,fontWeight:700,cursor:saving?"wait":"pointer" }}>
      ...
      <div style={{ background:"var(--bg-card)",borderRadius:12,overflow:"hidden" }}>
        {activeUsers.map(function(u) {
          ...
          return (
            <div key={u.id} style={{ padding:"16px 20px",borderBottom:"1px solid var(--border-light)" }}>
                ...
                <div style={{ display:"flex",alignItems:"center",gap:12 }}>
                  ...
                  <div>
                    <div style={{ display:"flex",alignItems:"center",gap:8 }}>
                      <span style={{ color:"var(--text-primary)",fontSize:14,fontWeight:700 }}>{u.name || u.email}</span>
                      ...
                      {isCurrentUser && <span style={{ padding:"2px 6px",borderRadius:4,fontSize:8,fontWeight:600,background:"#4ADE8018",color:"var(--green)" }}>YOU</span>}
                    </div>
                    <div style={{ color:"var(--text-muted)",fontSize:11,marginTop:2 }}>{u.email}</div>
                  </div>
                </div>
                <div style={{ display:"flex",gap:6,alignItems:"center" }}>
                  {!isCurrentUser && u.role !== "admin" && (
                    <button onClick={function(){handleUpdate(u.id, { dashboard_access: !u.dashboard_access });}}
                      style={{ padding:"5px 12px",borderRadius:6,border:"1px solid "+(u.dashboard_access?"#4ADE8033":"var(--border)"),background:u.dashboard_access?"#4ADE8012":"transparent",color:u.dashboard_access?"var(--green)":"var(--text-muted)",fontSize:10,cursor:"pointer",display:"flex",alignItems:"center",gap:4 }}>
                      <span style={{ width:8,height:8,borderRadius:"50%",background:u.dashboard_access?"var(--green)":"var(--text-muted)" }} />
                      {u.dashboard_access ? "Full Access" : "Store Only"}
                    </button>
                  )}
                  {u.role === "admin" && (
                    <span style={{ padding:"5px 12px",fontSize:10,color:"var(--green)" }}>Full Access</span>
                  )}
                  {onPreview && (
                    <button onClick={function(){onPreview(u.role, u.name, u.store);}}
                      style={{ padding:"5px 12px",borderRadius:6,border:"1px solid #7B2FFF22",background:"transparent",color:"var(--purple)",fontSize:10,cursor:"pointer" }}>
                      Preview
                    </button>
                  )}
                  {!isCurrentUser && (
                    <button onClick={function(){setEditingUser(isEditing ? null : u.id);}}
                      style={{ padding:"5px 12px",borderRadius:6,border:"1px solid var(--border)",background:"transparent",color:"var(--text-secondary)",fontSize:10,cursor:"pointer" }}>
                      {isEditing ? "Cancel" : "Edit"}
                    </button>
                  )}
                  {!isCurrentUser && (
                    <button onClick={function(){handleDeactivate(u.id, u.name || u.email);}}
                      style={{ padding:"5px 12px",borderRadius:6,border:"1px solid #F8717122",background:"transparent",color:"var(--red)",fontSize:10,cursor:"pointer" }}>
                      Revoke
                    </button>
                  )}
                </div>
              ...
              {/* Edit form */}
              {isEditing && (
                <div style={{ marginTop:12,padding:16,background:"var(--bg-card-inner)",borderRadius:8 }}>
                  <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:10 }}>
                    <div>
                      <label style={{ color:"var(--text-secondary)",fontSize:10,display:"block",marginBottom:3 }}>Name</label>
                      <input type="text" defaultValue={u.name}
                        id={"edit-name-"+u.id}
                        style={{ width:"100%",padding:"8px 10px",borderRadius:6,border:"1px solid var(--border)",background:"var(--bg-card)",color:"var(--text-primary)",fontSize:12,outline:"none",boxSizing:"border-box" }} />
                    </div>
                    <div>
                      <label style={{ color:"var(--text-secondary)",fontSize:10,display:"block",marginBottom:3 }}>Email</label>
                  ...
                  <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:10 }}>
                    <div>
                      <label style={{ color:"var(--text-secondary)",fontSize:10,display:"block",marginBottom:3 }}>Role</label>
                    ...
                    <div>
                      <label style={{ color:"var(--text-secondary)",fontSize:10,display:"block",marginBottom:3 }}>Store</label>
                  ...
                  <button onClick={function(){
                    ...
                    style={{ marginTop:10,padding:"7px 18px",borderRadius:6,border:"none",background:"var(--purple)",color:"#FFF",fontSize:11,fontWeight:700,cursor:"pointer" }}>
                    Save Changes
                  </button>
                </div>
              )}
        ...
        {activeUsers.length === 0 && (
          <div style={{ padding:40,textAlign:"center",color:"var(--text-muted)",fontSize:13 }}>No users configured.</div>
        )}
      </div>
      ...
      {inactiveUsers.length > 0 && (
        <div style={{ marginTop:24 }}>
          <div style={{ color:"var(--text-muted)",fontSize:13,fontWeight:700,marginBottom:8 }}>Deactivated Users ({inactiveUsers.length})</div>
          <div style={{ background:"var(--bg-card)",borderRadius:12,overflow:"hidden",opacity:0.6 }}>
            {inactiveUsers.map(function(u) {
              return (
                <div key={u.id} style={{ padding:"12px 20px",borderBottom:"1px solid var(--border-light)",display:"flex",justifyContent:"space-between",alignItems:"center" }}>
                  <div>
                    <span style={{ color:"var(--text-secondary)",fontSize:13 }}>{u.name || u.email}</span>
                    <span style={{ color:"var(--text-muted)",fontSize:11,marginLeft:8 }}>{u.email}</span>
                    <span style={{ color:"var(--red)",fontSize:9,marginLeft:8 }}>DEACTIVATED</span>
                  </div>
                  <button onClick={function(){handleReactivate(u.id);}}
                    style={{ padding:"4px 10px",borderRadius:4,border:"1px solid #4ADE8033",background:"transparent",color:"var(--green)",fontSize:10,cursor:"pointer" }}>
                    Reactivate
                  </button>
                </div>
              );
