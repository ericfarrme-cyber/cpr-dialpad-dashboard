...
const DataBanner = ({ isLive, isLoading, isStored, lastSync, onRefresh, onLiveRefresh }) => (
  <div style={{
    margin: "0 0 20px", padding: "10px 16px", borderRadius: 8,
    background: isStored ? "#7C8AFF12" : isLive ? "#4ADE8012" : "#FBBF2412",
    border: `1px solid ${isStored ? "#7C8AFF33" : isLive ? "#4ADE8033" : "#FBBF2433"}`,
    display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 8,
  }}>
    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
      <span style={{
        width: 8, height: 8, borderRadius: "50%",
        background: isStored ? "#7C8AFF" : isLive ? "#4ADE80" : "#FBBF24",
        animation: isLoading ? "pulse 1.5s infinite" : "none",
      }} />
      <span style={{ color: "#C8CAD0", fontSize: 12 }}>
        {isLoading ? "Fetching live data from Dialpad..." :
         isStored ? `Stored data · Last synced ${lastSync ? new Date(lastSync).toLocaleString() : "unknown"}` :
         isLive ? "Live data from Dialpad API" :
         "Sample data — waiting for first sync"}
      </span>
    </div>
    <div style={{ display: "flex", gap: 6 }}>
      {!isLoading && (
        <>
          <button onClick={onRefresh} style={{ padding: "4px 12px", borderRadius: 6, border: "1px solid #2A2D35", background: "transparent", color: "#8B8F98", fontSize: 11, cursor: "pointer", fontFamily: "'Space Grotesk', sans-serif" }}>↻ Reload</button>
          <button onClick={onLiveRefresh} style={{ padding: "4px 12px", borderRadius: 6, border: "1px solid #7C8AFF44", background: "#7C8AFF18", color: "#7C8AFF", fontSize: 11, cursor: "pointer", fontFamily: "'Space Grotesk', sans-serif" }}>⚡ Live Refresh</button>
        </>
      )}
    </div>
  </div>
...
const CustomTooltip = ({ active, payload, label }) => {
  ...
  return (
    <div style={{ background: "#1E2028", border: "1px solid #2A2D35", borderRadius: 8, padding: "10px 14px", boxShadow: "0 8px 32px rgba(0,0,0,0.4)" }}>
...
// ── OVERVIEW TAB (FIXED: uses _missed counts) ──
...
function KeywordsTab({ keywords }) {
  ...
  const filtered = useMemo(() => { let kw = keywords; if (cat !== "All") kw = kw.filter(k => k.category === cat); return [...kw].sort((a, b) => STORE_KEYS.reduce((s, k) => s + (b[k]||0), 0) - STORE_KEYS.reduce((s, k) => s + (a[k]||0), 0)); }, [cat, keywords]);
        ...
        <div style={{ background: "#1A1D23", borderRadius: 12, padding: 20 }}>
          ...
          <div style={{ maxHeight: 420, overflowY: "auto" }}>
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
              ...
              <tbody>{filtered.map((k, i) => { const total = STORE_KEYS.reduce((s, sk) => s + (k[sk]||0), 0); return (<tr key={i} style={{ borderBottom: "1px solid #1E2028" }}><td style={{ padding: "10px 10px", color: "#E8E9EC", fontSize: 13, fontWeight: 600 }}>{k.keyword}</td><td style={{ padding: "10px 6px" }}><span style={{ fontSize: 10, padding: "2px 8px", borderRadius: 4, background: (catColors[k.category]||"#8B8F98")+"18", color: catColors[k.category]||"#8B8F98", fontWeight: 600 }}>{k.category}</span></td>{STORE_KEYS.map(sk => <td key={sk} style={{ textAlign: "right", padding: "10px 6px", color: "#C8CAD0", fontSize: 13 }}>{k[sk]||0}</td>)}<td style={{ textAlign: "right", padding: "10px 10px", color: "#F0F1F3", fontSize: 13, fontWeight: 700 }}>{total}</td></tr>); })}</tbody>
            </table>
          </div>
        </div>
        <div><div style={{ background: "#1A1D23", borderRadius: 12, padding: 20 }}>
...
function CallbacksTab({ callbackData }) {
  return (
    <div>
      <div style={{ display: "grid", gridTemplateColumns: `repeat(${STORE_KEYS.length}, 1fr)`, gap: 14, marginBottom: 28 }}>
        {callbackData.map(cb => { const store = STORES[cb.store]; if (!store) return null; const rate = cb.missed > 0 ? ((cb.calledBack / cb.missed) * 100).toFixed(1) : "0.0"; return (
          <div key={cb.store} style={{ background: "#1A1D23", borderRadius: 12, padding: 20, border: `1px solid ${store.color}33` }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}><div style={{ color: "#F0F1F3", fontSize: 15, fontWeight: 700 }}>{store.name}</div><div style={{ padding: "4px 10px", borderRadius: 6, background: parseFloat(rate)>=80?"#4ADE8022":parseFloat(rate)>=60?"#FBBF2422":"#F8717122", color: parseFloat(rate)>=80?"#4ADE80":parseFloat(rate)>=60?"#FBBF24":"#F87171", fontSize: 14, fontWeight: 700 }}>{rate}%</div></div>
            {[{label:"Within 30 min",value:cb.within30,color:"#4ADE80"},{label:"30–60 min",value:cb.within60,color:"#FBBF24"},{label:"60+ min",value:cb.later,color:"#FB923C"},{label:"Never called back",value:cb.never,color:"#F87171"}].map((item,i) => (
              <div key={i} style={{ marginTop: 10 }}><div style={{ display: "flex", justifyContent: "space-between", marginBottom: 3 }}><span style={{ color: "#8B8F98", fontSize: 11 }}>{item.label}</span><span style={{ color: item.color, fontSize: 12, fontWeight: 700 }}>{item.value}</span></div><div style={{ background: "#12141A", borderRadius: 4, height: 6, overflow: "hidden" }}><div style={{ width: cb.missed>0?`${(item.value/cb.missed)*100}%`:"0%", height: "100%", background: item.color, borderRadius: 4 }} /></div></div>
            ))}
          </div>
        ); })}
      </div>
      <div style={{ background: "#1A1D23", borderRadius: 12, padding: 20 }}>
        ...
        <div style={{ height: 260 }}><ResponsiveContainer width="100%" height="100%"><BarChart data={callbackData.map(cb => ({ store: (STORES[cb.store]?.name||cb.store).replace("CPR ",""), "< 30 min": cb.within30, "30–60 min": cb.within60, "60+ min": cb.later, "Never": cb.never }))}><CartesianGrid strokeDasharray="3 3" stroke="#2A2D35" /><XAxis dataKey="store" tick={{ fill: "#8B8F98", fontSize: 12 }} tickLine={false} /><YAxis tick={{ fill: "#6B6F78", fontSize: 10 }} tickLine={false} axisLine={false} /><Tooltip content={<CustomTooltip />} /><Bar dataKey="< 30 min" stackId="a" fill="#4ADE80" /><Bar dataKey="30–60 min" stackId="a" fill="#FBBF24" /><Bar dataKey="60+ min" stackId="a" fill="#FB923C" /><Bar dataKey="Never" stackId="a" fill="#F87171" radius={[4,4,0,0]} /><Legend iconType="circle" iconSize={8} wrapperStyle={{ fontSize: 11 }} /></BarChart></ResponsiveContainer></div>
      </div>
    </div>
  );
}
  ...
  return (
    <div>
      ...
      <div style={{ background: "#1A1D23", borderRadius: 12, padding: 20 }}>
        ...
        <div style={{ height: 300 }}><ResponsiveContainer width="100%" height="100%"><BarChart data={problemCalls} layout="vertical" barGap={2}><CartesianGrid strokeDasharray="3 3" stroke="#2A2D35" horizontal={false} /><XAxis type="number" tick={{ fill: "#6B6F78", fontSize: 10 }} tickLine={false} axisLine={false} /><YAxis type="category" dataKey="type" tick={{ fill: "#8B8F98", fontSize: 10 }} width={140} tickLine={false} axisLine={false} /><Tooltip content={<CustomTooltip />} />{STORE_KEYS.map(k => <Bar key={k} dataKey={k} name={STORES[k].name.replace("CPR ","")} fill={STORES[k].color} radius={[0,4,4,0]} barSize={10} />)}</BarChart></ResponsiveContainer></div>
      </div>
    </div>
  );
}
...
// ═══════════════════════════════════════════════════════════════
// AUDIT TAB — with Batch "Audit All" support
// ═══════════════════════════════════════════════════════════════
...
function AuditTab({ rawCallData, storeFilter }) {
  ...
  const runAudit = async (call) => {
    ...
    try {
      ...
      if (json.success && json.audit) {
        setAudits(prev => [{ ...json.audit, call_id: call.call_id, date_started: call.date_started, store: json.audit.store, phone: call.external_number }, ...prev]);
        return true;
      } else {
        if (!json.alreadyAudited) setError(json.error || "Audit failed");
        return false;
      }
  ...
  const runBatchAudit = async () => {
    ...
    const toAudit = [...recordedCalls];
    setBatchProgress({ done: 0, total: toAudit.length });

    for (let i = 0; i < toAudit.length; i++) {
      if (batchAbort.current) break;
      setAuditingId(toAudit[i].call_id);
      await runAudit(toAudit[i]);
      setBatchProgress({ done: i + 1, total: toAudit.length });
      // Small delay between calls to avoid rate limits
      if (i < toAudit.length - 1) await new Promise(r => setTimeout(r, 1500));
    }
  ...
  if (loading) return <div style={{ padding: 40, textAlign: "center", color: "#6B6F78" }}><div style={{ fontSize: 24, marginBottom: 8, animation: "pulse 1.5s infinite" }}>🎯</div>Loading audit data...</div>;
        ...
        <div>
          ...
          {criteriaRates && (
            <div style={{ background: "#1A1D23", borderRadius: 12, padding: 20, marginBottom: 20 }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 16 }}>
                {[{ label: "Appointment Offered", rate: criteriaRates.appt, pts: "1.25 pts" }, { label: "Discount Mentioned", rate: criteriaRates.disc, pts: "0.92 pts" }, { label: "Lifetime Warranty", rate: criteriaRates.warr, pts: "0.92 pts" }, { label: "Faster Turnaround", rate: criteriaRates.turn, pts: "0.92 pts" }].map((item, i) => (
                  <div key={i}><div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}><span style={{ color: "#C8CAD0", fontSize: 12 }}>{item.label}</span><span style={{ color: parseInt(item.rate)>=70?"#4ADE80":parseInt(item.rate)>=40?"#FBBF24":"#F87171", fontSize: 13, fontWeight: 700 }}>{item.rate}%</span></div><div style={{ background: "#12141A", borderRadius: 4, height: 8, overflow: "hidden" }}><div style={{ width: `${item.rate}%`, height: "100%", borderRadius: 4, background: parseInt(item.rate)>=70?"#4ADE80":parseInt(item.rate)>=40?"#FBBF24":"#F87171" }} /></div><div style={{ color: "#6B6F78", fontSize: 10, marginTop: 4 }}>{item.pts}</div></div>
                ))}
              </div>
            </div>
          )}
          {storePerf.length > 0 && (
            <div style={{ display: "grid", gridTemplateColumns: `repeat(${storePerf.length}, 1fr)`, gap: 14 }}>
              {storePerf.map(sp => { const store = STORES[sp.store]; if (!store) return null; const scoreColor = sp.avg_score >= 3 ? "#4ADE80" : sp.avg_score >= 2 ? "#FBBF24" : "#F87171"; return (
                <div key={sp.store} style={{ background: "#1A1D23", borderRadius: 12, padding: 20, border: `1px solid ${store.color}33` }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 8 }}><div style={{ width: 32, height: 32, borderRadius: 8, background: store.color + "22", display: "flex", alignItems: "center", justifyContent: "center", color: store.color, fontWeight: 800, fontSize: 14 }}>{store.icon}</div><div><div style={{ color: "#F0F1F3", fontSize: 14, fontWeight: 700 }}>{store.name}</div><div style={{ color: "#6B6F78", fontSize: 11 }}>{sp.total_audits} audited</div></div></div>
                    <div style={{ padding: "6px 12px", borderRadius: 8, background: scoreColor + "22", color: scoreColor, fontSize: 18, fontWeight: 800 }}>{parseFloat(sp.avg_score).toFixed(2)}</div>
                  </div>
                  {[{ label: "Appt Offered", rate: sp.appt_rate }, { label: "Discount", rate: sp.discount_rate }, { label: "Warranty", rate: sp.warranty_rate }, { label: "Fast Turn.", rate: sp.turnaround_rate }].map((item, i) => (<div key={i} style={{ marginTop: 8 }}><div style={{ display: "flex", justifyContent: "space-between", marginBottom: 3 }}><span style={{ color: "#8B8F98", fontSize: 11 }}>{item.label}</span><span style={{ color: parseFloat(item.rate)>=70?"#4ADE80":parseFloat(item.rate)>=40?"#FBBF24":"#F87171", fontSize: 11, fontWeight: 700 }}>{parseFloat(item.rate).toFixed(0)}%</span></div><div style={{ background: "#12141A", borderRadius: 4, height: 6, overflow: "hidden" }}><div style={{ width: `${item.rate}%`, height: "100%", background: parseFloat(item.rate)>=70?"#4ADE80":parseFloat(item.rate)>=40?"#FBBF24":"#F87171", borderRadius: 4 }} /></div></div>))}
                </div>
              ); })}
            </div>
          )}
          {totalAudits === 0 && <div style={{ background: "#1A1D23", borderRadius: 12, padding: 40, textAlign: "center" }}><div style={{ fontSize: 32, marginBottom: 12 }}>🎯</div><div style={{ color: "#F0F1F3", fontSize: 16, fontWeight: 700, marginBottom: 8 }}>No audits yet</div><div style={{ color: "#6B6F78", fontSize: 13 }}>Go to "Audit Calls" to score your first call, or wait for the hourly auto-audit.</div></div>}
        </div>
      )}
          ...
          {employees.length > 0 ? (
            <div>
              <div style={{ background: "#1A1D23", borderRadius: 12, padding: 20 }}>
                <table style={{ width: "100%", borderCollapse: "collapse" }}>
                  <thead><tr style={{ borderBottom: "1px solid #2A2D35" }}>
                    {["Rank","Employee","Store","Calls","Avg Score","Appt %","Disc %","Warr %","Turn %"].map((h,i) => <th key={i} style={{ textAlign: i <= 2 ? "left" : "center", padding: "10px 12px", color: "#6B6F78", fontSize: 10, textTransform: "uppercase" }}>{h}</th>)}
                  </tr></thead>
                  <tbody>
                    {employees.sort((a, b) => b.avg_score - a.avg_score).map((emp, i) => {
                      const store = STORES[emp.store]; const scoreColor = emp.avg_score >= 3 ? "#4ADE80" : emp.avg_score >= 2 ? "#FBBF24" : "#F87171";
                      ...
                      return (<tr key={`${emp.employee}-${emp.store}`} style={{ borderBottom: "1px solid #1E2028" }}>
                        ...
                        <td style={{ textAlign: "center", padding: "12px" }}><span style={{ padding: "4px 10px", borderRadius: 6, background: scoreColor + "22", color: scoreColor, fontSize: 14, fontWeight: 800 }}>{parseFloat(emp.avg_score).toFixed(2)}</span></td>
                        {["appt_rate", "discount_rate", "warranty_rate", "turnaround_rate"].map(key => { const val = parseFloat(emp[key]); return <td key={key} style={{ textAlign: "center", padding: "12px", color: val >= 70 ? "#4ADE80" : val >= 40 ? "#FBBF24" : "#F87171", fontSize: 13, fontWeight: 600 }}>{val.toFixed(0)}%</td>; })}
                      </tr>);
                    })}
                  </tbody>
                </table>
              ...
              <div style={{ background: "#1A1D23", borderRadius: 12, padding: 20, marginTop: 20 }}>
                ...
                <div style={{ height: 300 }}><ResponsiveContainer width="100%" height="100%"><BarChart data={employees.sort((a,b) => b.avg_score - a.avg_score).slice(0,10).map(e => ({ name: e.employee, score: parseFloat(e.avg_score) }))} layout="vertical"><CartesianGrid strokeDasharray="3 3" stroke="#2A2D35" horizontal={false} /><XAxis type="number" domain={[0, 4]} tick={{ fill: "#6B6F78", fontSize: 10 }} tickLine={false} axisLine={false} /><YAxis type="category" dataKey="name" tick={{ fill: "#C8CAD0", fontSize: 12 }} width={100} tickLine={false} axisLine={false} /><Tooltip content={<CustomTooltip />} /><Bar dataKey="score" name="Avg Score" fill="#7C8AFF" radius={[0,6,6,0]} barSize={18} /></BarChart></ResponsiveContainer></div>
              </div>
            </div>
          ) : <div style={{ background: "#1A1D23", borderRadius: 12, padding: 40, textAlign: "center", color: "#6B6F78" }}>No employee data yet. Audit some calls to build the leaderboard.</div>}
        </div>
      )}

      {/* AUDIT CALLS — with Audit All button */}
      {auditView === "calls" && (
        <div>
          ...
          {batchRunning && (
            <div style={{ background: "#1A1D23", borderRadius: 8, padding: "12px 16px", marginBottom: 16, border: "1px solid #7C8AFF33" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
                <span style={{ color: "#C8CAD0", fontSize: 13, fontWeight: 600 }}>
                  Batch Audit: {batchProgress.done} / {batchProgress.total} calls scored
                </span>
                <button onClick={stopBatch} style={{ padding: "4px 12px", borderRadius: 6, border: "1px solid #F8717144", background: "#F8717118", color: "#F87171", fontSize: 11, cursor: "pointer", fontWeight: 600 }}>Stop</button>
              </div>
              <div style={{ background: "#12141A", borderRadius: 4, height: 8, overflow: "hidden" }}>
                <div style={{ width: `${batchProgress.total > 0 ? (batchProgress.done / batchProgress.total) * 100 : 0}%`, height: "100%", background: "#7C8AFF", borderRadius: 4, transition: "width 0.3s" }} />
              </div>
            </div>
          )}
              ...
              {recordedCalls.map((call, i) => {
                ...
                return (
                  <div key={i} style={{ padding: "12px 0", borderBottom: "1px solid #2A2D35", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    <div>
                      <div style={{ display: "flex", alignItems: "center", gap: 8 }}><span style={{ width: 8, height: 8, borderRadius: "50%", background: store?.color || "#8B8F98" }} /><span style={{ color: "#E8E9EC", fontSize: 13, fontWeight: 600 }}>{call.external_number}</span><span style={{ color: "#6B6F78", fontSize: 11 }}>→ {call.name}</span></div>
                    ...
                    <button onClick={() => !isAuditing && !batchRunning && runAudit(call)} disabled={isAuditing || batchRunning} style={{ padding: "6px 14px", borderRadius: 6, border: "none", cursor: isAuditing || batchRunning ? "default" : "pointer", background: isAuditing ? "#7C8AFF22" : "#7C8AFF", color: isAuditing ? "#7C8AFF" : "#FFF", fontSize: 12, fontWeight: 600, animation: isAuditing ? "pulse 1.5s infinite" : "none" }}>{isAuditing ? "Scoring..." : "Audit"}</button>
                  </div>
                );
              })}
              {recordedCalls.length === 0 && <div style={{ color: "#6B6F78", fontSize: 13, padding: 20, textAlign: "center" }}>All recorded calls have been audited! New calls will be auto-scored every hour.</div>}
            </div>
          </div>
        </div>
      )}
      ...
      {auditView === "history" && (
        <div>
          ...
          <div style={{ background: "#1A1D23", borderRadius: 12, padding: 20 }}>
            <div style={{ maxHeight: 600, overflowY: "auto" }}>
              {audits.map((audit, i) => {
                const score = parseFloat(audit.score || 0); const scoreColor = score >= 3 ? "#4ADE80" : score >= 2 ? "#FBBF24" : "#F87171";
                const store = STORES[audit.store]; const d = new Date(audit.date_started || audit.date);
                return (
                  <div key={audit.call_id || i} style={{ padding: 16, borderBottom: "1px solid #2A2D35" }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
                      <div><div style={{ color: "#E8E9EC", fontSize: 13, fontWeight: 700 }}>{audit.employee || "Unknown"} — {audit.phone}</div><div style={{ color: "#6B6F78", fontSize: 11 }}>{d.toLocaleDateString()} {d.toLocaleTimeString([], {hour:'2-digit',minute:'2-digit'})} · <span style={{ color: store?.color }}>{store?.name || audit.store}</span></div></div>
                      <div style={{ padding: "6px 12px", borderRadius: 8, background: scoreColor + "22", color: scoreColor, fontSize: 16, fontWeight: 800 }}>{score.toFixed(2)} / 4</div>
                    </div>
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", gap: 6 }}>
                      {[{ key: "appt_offered", label: "Appt", notes: "appt_notes" }, { key: "discount_mentioned", label: "Discount", notes: "discount_notes" }, { key: "warranty_mentioned", label: "Warranty", notes: "warranty_notes" }, { key: "faster_turnaround", label: "Fast Turn.", notes: "turnaround_notes" }].map(({ key, label, notes }) => (
              ...
              {audits.length === 0 && <div style={{ color: "#6B6F78", fontSize: 13, padding: 20, textAlign: "center" }}>No audit history yet.</div>}
            </div>
          </div>
        </div>
      )}
...
export default function DialpadDashboard() {
  ...
  const loadStoredData = useCallback(async () => {
    try {
      const res = await fetch("/api/dialpad/stored?days=30");
      const json = await res.json();
      if (json.success && json.hasData) {
        const d = json.data;
        if (d.dailyCalls?.length > 0) setDailyCalls(d.dailyCalls);
        if (d.hourlyMissed?.length > 0) setHourlyMissed(d.hourlyMissed);
        if (d.dowData?.length > 0) setDowData(d.dowData);
        if (d.callbackData?.length > 0) setCallbackData(d.callbackData);
        if (d.problemCalls?.length > 0) setProblemCalls(d.problemCalls);
        setIsStored(true); setLastSync(json.lastSync);
        return true;
      }
    } catch (err) { console.error("Failed to load stored data:", err); }
    return false;
  }, []);
  ...
  const loadLiveData = useCallback(async () => {
    ...
    try {
      ...
      if (data && data.length > 0) {
        ...
        const callbacks = transformToCallbackData(data); if (callbacks.some(c => c.missed > 0)) setCallbackData(callbacks);
        const problems = transformToProblemCalls(data); if (problems.some(p => STORE_KEYS.some(k => p[k] > 0))) setProblemCalls(problems);
  ...
  useEffect(() => {
    async function init() {
      const hasStored = await loadStoredData();
      if (!hasStored) await loadLiveData();
    }
    init();
  }, [loadStoredData, loadLiveData]);

  // FIXED: Use _missed counts from data instead of total - answered
  const overviewStats = useMemo(() => {
    const totals = { total: 0, answered: 0, missed: 0 };
    const storeStats = {};
    STORE_KEYS.forEach(s => { storeStats[s] = { total: 0, answered: 0, missed: 0 }; });
    dailyCalls.forEach(d => {
      STORE_KEYS.forEach(s => {
        const t = d[`${s}_total`] || 0;
        const a = d[`${s}_answered`] || 0;
        const m = d[`${s}_missed`] !== undefined ? d[`${s}_missed`] : (t - a);
        storeStats[s].total += t;
        storeStats[s].answered += a;
        storeStats[s].missed += m;
        totals.total += t;
        totals.answered += a;
        totals.missed += m;
      });
    ...
    <div style={{ background: "#0F1117", minHeight: "100vh", color: "#F0F1F3", fontFamily: "'Space Grotesk', -apple-system, sans-serif" }}>
      ...
      <div style={{ background: "#12141A", borderBottom: "1px solid #1E2028", padding: "16px 28px", display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 12 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
          ...
          <div><h1 style={{ margin: 0, fontSize: 19, fontWeight: 800, letterSpacing: "-0.02em" }}>Dialpad Analytics</h1><p style={{ margin: 0, color: "#6B6F78", fontSize: 12 }}>CPR Store Call Intelligence · Last 30 Days</p></div>
        </div>
      ...
      <div style={{ background: "#12141A", borderBottom: "1px solid #1E2028", padding: "0 28px", display: "flex", gap: 0, overflowX: "auto" }}>
        {TABS.map(tab => (
          <button key={tab.id} onClick={() => setActiveTab(tab.id)} style={{ padding: "14px 20px", border: "none", cursor: "pointer", background: "transparent", color: activeTab === tab.id ? "#F0F1F3" : "#6B6F78", fontSize: 13, fontWeight: 600, borderBottom: activeTab === tab.id ? "2px solid #7C8AFF" : "2px solid transparent", display: "flex", alignItems: "center", gap: 6, whiteSpace: "nowrap", fontFamily: "'Space Grotesk', sans-serif" }}><span style={{ fontSize: 14 }}>{tab.icon}</span>{tab.label}</button>
        ))}
      </div>
      ...
      <div style={{ padding: "16px 28px", borderTop: "1px solid #1E2028", color: "#4A4D55", fontSize: 11, textAlign: "center", fontFamily: "'JetBrains Mono', monospace" }}>
        {isStored ? `Stored data · Last synced ${lastSync ? new Date(lastSync).toLocaleString() : "—"}` : isLive ? "Live data from Dialpad API" : "Sample data"} · Focused Technologies LLC
      </div>
    </div>
  );
}
