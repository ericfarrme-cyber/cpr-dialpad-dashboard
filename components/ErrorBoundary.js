...
export default class ErrorBoundary extends Component {
  ...
  render() {
    if (this.state.hasError) {
      ...
      return (
        <div style={{ background: "var(--bg-page)", minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'Space Grotesk', -apple-system, sans-serif" }}>
          <div style={{ background: "var(--bg-card)", borderRadius: 16, padding: 40, maxWidth: 520, width: "90%", textAlign: "center", border: "1px solid var(--border)" }}>
            <div style={{ fontSize: 48, marginBottom: 16 }}>{isExtensionError ? "\uD83D\uDD0C" : "\u26A0\uFE0F"}</div>
            <h2 style={{ color: "var(--text-primary)", fontSize: 20, fontWeight: 800, margin: "0 0 8px" }}>
            ...
            <div style={{ display: "flex", gap: 10, justifyContent: "center", flexWrap: "wrap" }}>
              <button
                ...
                style={{ padding: "10px 24px", borderRadius: 8, border: "none", background: "linear-gradient(135deg, var(--purple), var(--cyan))", color: "#FFF", fontSize: 13, fontWeight: 700, cursor: "pointer" }}>
                Try Again
              </button>
              <button
                onClick={function() { window.location.reload(); }}
                style={{ padding: "10px 24px", borderRadius: 8, border: "1px solid var(--border)", background: "transparent", color: "var(--text-secondary)", fontSize: 13, fontWeight: 600, cursor: "pointer" }}>
                Reload Page
              </button>
            </div>
            ...
            {!isExtensionError && this.state.error && (
              <div style={{ marginTop: 16, padding: 12, background: "var(--bg-card-inner)", borderRadius: 8, textAlign: "left" }}>
                <div style={{ color: "var(--red)", fontSize: 11, fontFamily: "monospace", wordBreak: "break-all" }}>
