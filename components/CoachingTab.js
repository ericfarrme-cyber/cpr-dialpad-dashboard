...
var SECTIONS = [
  { id: "regression", title: "🚨 Needs Attention", subtitle: "Coach now — performance issues that hurt this period's results", color: "var(--red)" },
  { id: "win", title: "🌟 Recognize Today", subtitle: "Catch them doing it right — give the feedback while it's fresh", color: "var(--green)" },
  { id: "opportunity", title: "⚡ Active Now", subtitle: "On shift today and below baseline — intervention possible", color: "var(--yellow)" },
];
...
export default function CoachingTab() {
  ...
  return (
    <div style={{ padding: 24, color: "var(--text-primary)" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 20 }}>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          ...
          <button onClick={runDetection} disabled={computing}
            style={Object.assign({}, btnBase, { background: "linear-gradient(135deg, var(--purple), var(--pink))", border: "none", color: "#fff", cursor: computing ? "wait" : "pointer", opacity: computing ? 0.6 : 1 })}>
        ...
        {SECTIONS.map(function(s) {
          ...
          return (
            <div key={s.id} style={Object.assign({}, card, { borderTop: "3px solid " + s.color, padding: 14 })}>
              <div style={{ color: "var(--text-secondary)", fontSize: 10, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.05em" }}>{s.title}</div>
              <div style={{ color: s.color, fontSize: 28, fontWeight: 800, marginTop: 4 }}>{n}</div>
              <div style={{ color: "var(--text-muted)", fontSize: 10 }}>{n === 1 ? "active flag" : "active flags"}</div>
            </div>
          );
        })}
      </div>
      ...
      {loading ? (
        <div style={{ color: "var(--text-secondary)", textAlign: "center", padding: 40 }}>Loading flags…</div>
      ) : totalFlags === 0 ? (
        <div style={{ ...card, textAlign: "center", padding: 60 }}>
          <div style={{ fontSize: 40, marginBottom: 8 }}>{"\uD83D\uDC4C"}</div>
          <div style={{ fontSize: 16, fontWeight: 700, color: "var(--text-primary)", marginBottom: 6 }}>All clear</div>
          <div style={{ color: "var(--text-secondary)", fontSize: 12 }}>No active flags right now. Click <strong>Run Detection</strong> to refresh, or wait for the cron to fire.</div>
        </div>
      ) : (
        SECTIONS.map(function(s) {
          ...
          return (
            <div key={s.id} style={{ marginBottom: 28 }}>
              <div style={{ display: "flex", alignItems: "baseline", gap: 10, marginBottom: 12 }}>
                ...
                <div style={{ marginLeft: "auto", fontSize: 11, color: "var(--text-muted)" }}>{sectionFlags.length} active</div>
              </div>
      ...
      {activeModal && (
        <div onClick={function() { if (!activeModal.sending) setActiveModal(null); }}
          ...
          <div onClick={function(e) { e.stopPropagation(); }}
            style={{ background: "var(--bg-page)", borderRadius: 12, maxWidth: 600, width: "100%", border: "1px solid #2A2D36", overflow: "hidden", maxHeight: "90vh", display: "flex", flexDirection: "column" }}>
            <div style={{ padding: 20, borderBottom: "1px solid var(--border-light)" }}>
              ...
              <div style={{ color: "var(--text-secondary)", fontSize: 12, marginTop: 4 }}>
                <strong style={{ color: "var(--text-primary)" }}>{activeModal.flag.employee_name}</strong>
              ...
              <div style={{ color: "var(--text-muted)", fontSize: 11, marginTop: 6, fontStyle: "italic" }}>
            ...
            <div style={{ padding: 20, flex: 1, overflowY: "auto" }}>
              {activeModal.drafting ? (
                ...
                <div style={{ color: "var(--red)", padding: 14, background: "#F8717111", borderRadius: 6, fontSize: 12 }}>{activeModal.error}</div>
              ) : (
                <div>
                  <label style={{ display: "block", color: "var(--text-secondary)", fontSize: 11, fontWeight: 700, marginBottom: 6, textTransform: "uppercase", letterSpacing: "0.05em" }}>Message (edit before sending)</label>
                  <textarea value={activeModal.draft || ""} onChange={function(e) { setActiveModal(function(m) { return Object.assign({}, m, { draft: e.target.value }); }); }}
                    rows={6}
                    style={{ width: "100%", padding: 12, borderRadius: 6, background: "var(--bg-card-inner)", color: "var(--text-primary)", border: "1px solid #2A2D36", fontSize: 13, fontFamily: "inherit", resize: "vertical", lineHeight: 1.5 }} />

                  <div style={{ marginTop: 16 }}>
                    <label style={{ display: "block", color: "var(--text-secondary)", fontSize: 11, fontWeight: 700, marginBottom: 8, textTransform: "uppercase", letterSpacing: "0.05em" }}>Delivery Method</label>
                    <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                      {activeModal.flag.flag_type === "win" && (
                        <label style={radioRow(activeModal.deliveryMethod === "slack")}>
                          ...
                          <span><strong style={{ color: "var(--green)" }}>Post to #wins</strong> — public team-wide recognition</span>
                        </label>
                      )}
                      <label style={radioRow(activeModal.deliveryMethod === "note")}>
                        <input type="radio" name="delivery" value="note" checked={activeModal.deliveryMethod === "note"}
                          ...
                          style={{ marginRight: 10 }} />
                        <span><strong style={{ color: "var(--purple)" }}>Push to their MyPerformance</strong> — private, they see it next login with an obvious badge</span>
                      </label>
                      <label style={radioRow(activeModal.deliveryMethod === "1on1")}>
                        <input type="radio" name="delivery" value="1on1" checked={activeModal.deliveryMethod === "1on1"}
                          ...
                          style={{ marginRight: 10 }} />
                        <span><strong style={{ color: "var(--yellow)" }}>Save for 1:1</strong> — marks the flag as acted on, saves the message for your reference</span>
                      </label>
                    </div>
                  </div>
                </div>
            ...
            <div style={{ padding: 16, borderTop: "1px solid var(--border-light)", display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8 }}>
              ...
              <div style={{ display: "flex", gap: 8 }}>
                ...
                <button onClick={sendCoachMessage} disabled={!activeModal.draft || activeModal.sending}
                  style={Object.assign({}, btnBase, { background: "linear-gradient(135deg, var(--purple), var(--pink))", border: "none", color: "#fff", opacity: activeModal.draft && !activeModal.sending ? 1 : 0.5 })}>
...
function radioRow(checked) {
  return {
    ...
    background: checked ? "#7B2FFF15" : "var(--bg-card-inner)",
    border: "1px solid " + (checked ? "#7B2FFF55" : "#2A2D36"),
    cursor: "pointer", fontSize: 12, color: "var(--text-primary)", lineHeight: 1.4,
  };
}
...
function FlagCard(props) {
  ...
  var sevColor = flag.severity >= 5 ? "var(--red)" : flag.severity >= 4 ? "var(--orange)" : flag.severity >= 3 ? "var(--yellow)" : "var(--text-secondary)";
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 12 }}>
        <div style={{ flex: 1 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
            ...
            <span style={{ fontSize: 14, fontWeight: 700, color: "var(--text-primary)" }}>{flag.employee_name}</span>
            {store && <span style={{ fontSize: 11, color: store.color, fontWeight: 600 }}>· {store.name.replace("CPR ", "")}</span>}
          </div>
          <div style={{ fontSize: 13, color: "var(--text-primary)", marginBottom: 6, lineHeight: 1.4 }}>{flag.headline}</div>
          {(flag.metric_current != null && flag.metric_baseline != null) && (
            <div style={{ fontSize: 11, color: "var(--text-secondary)", marginBottom: 4 }}>
              {flag.metric_label}: <strong style={{ color: "var(--text-primary)" }}>{flag.metric_current}</strong>
              {flag.metric_baseline > 0 && <span> · baseline {flag.metric_baseline}</span>}
              {flag.delta != null && <span style={{ color: flag.delta < 0 ? "var(--red)" : "var(--green)", marginLeft: 6, fontWeight: 700 }}>({flag.delta > 0 ? "+" : ""}{flag.delta})</span>}
            </div>
          )}
          ...
          {ev.top_gaps && ev.top_gaps.length > 0 && (
            <div style={{ marginTop: 8, padding: "8px 10px", borderRadius: 6, background: "#FB923C18", borderLeft: "3px solid var(--orange)" }}>
              <div style={{ fontSize: 10, color: "var(--orange)", fontWeight: 800, letterSpacing: "0.05em", textTransform: "uppercase", marginBottom: 4 }}>{"\u26A0"} Specific gaps</div>
              ...
              {ev.ticket_details.slice(0, 4).map(function(td) {
                return (
                  <div key={td.ticket_number} style={{ padding: "4px 0", display: "flex", gap: 8, alignItems: "baseline", flexWrap: "wrap" }}>
                    <a href={"https://cpr.repairq.io/ticket/" + td.ticket_number} target="_blank" rel="noopener noreferrer" onClick={function(e) { e.stopPropagation(); }} style={{ color: "var(--cyan)", textDecoration: "none", fontWeight: 700 }}>#{td.ticket_number}</a>
                    {td.notes_score != null && <span style={{ color: td.notes_score < 30 ? "var(--red)" : td.notes_score < 50 ? "var(--orange)" : "var(--yellow)", fontWeight: 700 }}>notes {td.notes_score}/100</span>}
                    {td.gaps && td.gaps.length > 0 && <span style={{ color: "var(--text-muted)", fontStyle: "italic" }}>missing: {td.gaps.join(", ")}</span>}
                  </div>
                );
              })}
              {ev.ticket_details.length > 4 && <div style={{ marginTop: 4, color: "var(--text-muted)" }}>+ {ev.ticket_details.length - 4} more</div>}
            </div>
          ) : ev.ticket_numbers && ev.ticket_numbers.length > 0 ? (
            <div style={{ fontSize: 10, color: "var(--text-muted)", marginTop: 6 }}>
              Tickets: {ev.ticket_numbers.map(function(tn, i) {
                return <span key={tn}>{i > 0 ? ", " : ""}<a href={"https://cpr.repairq.io/ticket/" + tn} target="_blank" rel="noopener noreferrer" onClick={function(e) { e.stopPropagation(); }} style={{ color: "var(--cyan)", textDecoration: "none" }}>#{tn}</a></span>;
              })}
            </div>
          ) : null}

          {ev.attribution_note && (
            <div style={{ fontSize: 9, color: "var(--text-muted)", marginTop: 6, fontStyle: "italic" }}>{ev.attribution_note}</div>
          )}
        </div>
        <div style={{ display: "flex", flexDirection: "column", gap: 6, flexShrink: 0 }}>
          <button onClick={function(e) { e.stopPropagation(); props.onCoach(flag); }}
            style={Object.assign({}, btnBase, { background: "linear-gradient(135deg, var(--purple), var(--pink))", border: "none", color: "#fff", whiteSpace: "nowrap" })}>
          ...
          <div style={{ fontSize: 9, color: "var(--text-muted)", textAlign: "center", marginTop: 2 }}>Click for details</div>
        </div>
      </div>
    </div>
  );
...
function DetailModal(props) {
  ...
  var typeColor = isWin ? "var(--green)" : flag.flag_type === "opportunity" ? "var(--yellow)" : "var(--red)";
  ...
  return (
    <div onClick={props.onClose}
      style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.7)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 100, padding: 20 }}>
      <div onClick={function(e) { e.stopPropagation(); }}
        style={{ background: "var(--bg-page)", borderRadius: 12, maxWidth: 760, width: "100%", border: "1px solid #2A2D36", overflow: "hidden", maxHeight: "92vh", display: "flex", flexDirection: "column" }}>

        {/* Header */}
        <div style={{ padding: "20px 24px", borderBottom: "1px solid var(--border-light)", borderTop: "3px solid " + typeColor }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6 }}>
            ...
            <span style={{ display: "inline-block", width: 6, height: 6, borderRadius: "50%", background: sevColor }} />
            <span style={{ fontSize: 10, color: "var(--text-secondary)", textTransform: "uppercase", letterSpacing: "0.05em" }}>severity {flag.severity || 3}/5</span>
            <span style={{ marginLeft: "auto", fontSize: 10, color: "var(--text-muted)" }}>Detected {fmtDateTime(flag.created_at)}</span>
          </div>
          <div style={{ fontSize: 20, fontWeight: 800, color: "var(--text-primary)", marginBottom: 4 }}>{flag.employee_name}</div>
          {store && <div style={{ fontSize: 12, color: store.color, fontWeight: 600, marginBottom: 10 }}>{store.name}</div>}
          <div style={{ fontSize: 14, color: "var(--text-primary)", lineHeight: 1.5 }}>{flag.headline}</div>
        </div>
        ...
        <div style={{ padding: 24, flex: 1, overflowY: "auto" }}>
          ...
          {(flag.metric_current != null || flag.metric_baseline != null) && (
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12, marginBottom: 24 }}>
              <Stat label={flag.metric_label || "Current"} value={flag.metric_current != null ? flag.metric_current : "\u2014"} color="var(--text-primary)" />
              <Stat label="Baseline" value={flag.metric_baseline != null && flag.metric_baseline > 0 ? flag.metric_baseline : "\u2014"} color="var(--text-secondary)" />
              {flag.delta != null && <Stat label="Delta" value={(flag.delta > 0 ? "+" : "") + flag.delta} color={flag.delta < 0 ? "var(--red)" : "var(--green)"} />}
            </div>
          )}
          ...
          {flag.rule_key === "notes_streak" && (
            <div>
                ...
                <Section title="What's Missing">
                  <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                    {ev.all_gaps && ev.all_gaps.length > 0 ? ev.all_gaps.map(function(g) {
                      return (
                        <div key={g.gap} style={{ display: "flex", justifyContent: "space-between", padding: "8px 12px", background: "#FB923C12", borderLeft: "3px solid var(--orange)", borderRadius: 4 }}>
                          <span style={{ color: "var(--text-primary)", fontSize: 12 }}>{g.gap}</span>
                          <span style={{ color: "var(--orange)", fontSize: 11, fontWeight: 700 }}>{g.count} ticket{g.count === 1 ? "" : "s"}</span>
                        </div>
                      );
                    }) : ev.top_gaps.map(function(g) {
                      return <div key={g} style={{ padding: "8px 12px", background: "#FB923C12", borderLeft: "3px solid var(--orange)", borderRadius: 4, color: "var(--text-primary)", fontSize: 12 }}>{g}</div>;
                    })}
                  </div>
                </Section>
              )}
              ...
              {ev.attribution_note && (
                <div style={{ marginTop: 16, padding: 12, background: "#7B2FFF11", borderLeft: "3px solid var(--purple)", borderRadius: 4, fontSize: 11, color: "var(--text-secondary)" }}>
                  <strong style={{ color: "var(--purple)" }}>Attribution:</strong> {ev.attribution_note}
                </div>
              )}
            </div>
          )}
          ...
          {flag.rule_key === "appt_collapse" && (
            <div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 16 }}>
                <Stat label="This Week" value={ev.offers_current + " / " + ev.opps_current_week} sub={"appointment offers"} color="var(--red)" />
                <Stat label="Prior Week" value={ev.offers_prior + " / " + ev.opps_prior_week} sub={"appointment offers"} color="var(--green)" />
              </div>
                    ...
                    {ev.call_breakdown.map(function(c, i) {
                      return (
                        <tr key={i}>
                          ...
                          <Td>{c.offered ? <span style={{ color: "var(--green)", fontWeight: 700 }}>{"\u2713 Yes"}</span> : <span style={{ color: "var(--red)", fontWeight: 700 }}>{"\u2717 No"}</span>}</Td>
                          <Td>{c.warranty_mentioned ? <span style={{ color: "var(--green)" }}>{"\u2713"}</span> : <span style={{ color: "var(--text-muted)" }}>{"\u2014"}</span>}</Td>
                        </tr>
                      );
                    })}
                  </Table>
          ...
          {flag.rule_key === "active_shift_dip" && (
            <div>
              <div style={{ padding: "10px 14px", background: "#FBBF2412", borderLeft: "3px solid var(--yellow)", borderRadius: 4, marginBottom: 16, fontSize: 12, color: "var(--text-primary)" }}>
                {"\u26A1"} <strong style={{ color: "var(--yellow)" }}>{flag.employee_name} is on shift right now.</strong> Coach today, while the calls are still happening.
              </div>
                    ...
                    {ev.today_calls.map(function(c, i) {
                      return (
                        <tr key={i}>
                          ...
                          <Td>{c.appt_offered ? <span style={{ color: "var(--green)" }}>{"\u2713"}</span> : <span style={{ color: "var(--red)" }}>{"\u2717"}</span>}</Td>
                          <Td>{c.warranty_mentioned ? <span style={{ color: "var(--green)" }}>{"\u2713"}</span> : <span style={{ color: "var(--text-muted)" }}>{"\u2014"}</span>}</Td>
                        </tr>
                      );
                    })}
          ...
          {flag.rule_key === "turnaround_improvement" && (
            <div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12, marginBottom: 16 }}>
                <Stat label="Last 14 Days Avg" value={ev.avg_recent_hours + "h"} sub={ev.recent_window_tix + " " + ev.device_category + " tickets"} color="var(--green)" />
                <Stat label="Prior 14 Days" value={ev.avg_prior_hours + "h"} sub={ev.prior_window_tix + " tickets"} color="var(--text-secondary)" />
                <Stat label="Improvement" value={ev.pct_improvement + "% faster"} color="var(--green)" />
              </div>
                ...
                <Section title={"\uD83D\uDD25 Fastest Recent " + ev.device_category + " Repairs"}>
                  <Table headers={["Ticket", "Date", "Turnaround"]}>
                    {ev.sample_recent.map(function(t) {
                      return <tr key={t.ticket_number}><Td><TicketLink num={t.ticket_number} /></Td><Td muted>{fmtDate(t.date_closed)}</Td><Td><span style={{ color: "var(--green)", fontWeight: 700 }}>{t.hours}h</span></Td></tr>;
                    })}
                  </Table>
                </Section>
              )}
                ...
                <Section title="For Comparison: Slower Prior Repairs">
                  <Table headers={["Ticket", "Date", "Turnaround"]}>
                    {ev.sample_prior.map(function(t) {
                      return <tr key={t.ticket_number}><Td><TicketLink num={t.ticket_number} /></Td><Td muted>{fmtDate(t.date_closed)}</Td><Td><span style={{ color: "var(--orange)" }}>{t.hours}h</span></Td></tr>;
                    })}
                  </Table>
                </Section>
              )}
          ...
          {flag.rule_key === "appt_streak" && (
            <div>
              <div style={{ padding: "12px 14px", background: "#4ADE8012", borderLeft: "3px solid var(--green)", borderRadius: 4, marginBottom: 16, fontSize: 13, color: "var(--text-primary)" }}>
                {"\uD83C\uDFAF"} <strong style={{ color: "var(--green)" }}>{ev.streak_length} calls in a row</strong> with appointment offered. That's the kind of consistency worth calling out.
              </div>
                ...
                {ev.streak_calls && ev.streak_calls.length > 0 ? (
                  <Table headers={["#", "Date", "Score", "Warranty Mentioned?"]}>
                    {ev.streak_calls.map(function(c, i) {
                      return (
                        <tr key={i}>
                          <Td><strong style={{ color: "var(--green)" }}>{i + 1}</strong></Td>
                          <Td muted>{fmtDateTime(c.date)}</Td>
                          <Td><ScorePill score={c.score} /></Td>
                          <Td>{c.warranty_mentioned ? <span style={{ color: "var(--green)" }}>{"\u2713"}</span> : <span style={{ color: "var(--text-muted)" }}>{"\u2014"}</span>}</Td>
                        </tr>
          ...
          {flag.rule_key === "notes_excellence" && (
            <div>
              <div style={{ padding: "12px 14px", background: "#4ADE8012", borderLeft: "3px solid var(--green)", borderRadius: 4, marginBottom: 16, fontSize: 13, color: "var(--text-primary)" }}>
                {"\u2728"} <strong style={{ color: "var(--green)" }}>{ev.streak_length} consecutive tickets</strong> with notes scoring 90+ (avg {ev.avg_streak_score}/100). This is the documentation discipline you want everyone copying.
              </div>
          ...
          {flag.rule_key === "tier_crossover" && (
            <div>
              <div style={{ padding: "16px 18px", background: "linear-gradient(90deg, #4ADE8015, #00D4FF15)", borderRadius: 8, marginBottom: 16, textAlign: "center" }}>
                <div style={{ fontSize: 11, color: "var(--text-secondary)", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.05em" }}>Tier Movement · {ev.period}</div>
                <div style={{ fontSize: 22, fontWeight: 800, color: "var(--text-primary)", marginTop: 6 }}>
                  {ev.from_tier} {"\u2192"} <span style={{ color: "var(--green)" }}>{ev.to_tier}</span>
                </div>
                {flag.metric_current != null && (
                  <div style={{ fontSize: 12, color: "var(--text-secondary)", marginTop: 4 }}>
                    Score: {flag.metric_baseline} {"\u2192"} <strong style={{ color: "var(--text-primary)" }}>{flag.metric_current}</strong>
                  </div>
                )}
              </div>
            </div>
          ...
          {["notes_streak", "compliance_regression", "appt_collapse", "active_shift_dip", "turnaround_improvement", "appt_streak", "notes_excellence", "tier_crossover"].indexOf(flag.rule_key) < 0 && (
            <Section title="Evidence">
              <pre style={{ fontSize: 11, color: "var(--text-secondary)", background: "var(--bg-card-inner)", padding: 12, borderRadius: 6, overflow: "auto", maxHeight: 300 }}>{JSON.stringify(ev, null, 2)}</pre>
            </Section>
          )}
        </div>
        ...
        <div style={{ padding: 16, borderTop: "1px solid var(--border-light)", display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8 }}>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            <button onClick={function() { props.onMark1on1(flag); }} style={Object.assign({}, btnBase, { color: "var(--yellow)", borderColor: "#FBBF2455" })}>
              {"\uD83D\uDDD3"} Save for 1:1
            </button>
            <button onClick={function() { props.onDismiss(flag); }} style={btnBase}>Dismiss</button>
            <button onClick={function() { props.onCoach(flag); }}
              style={Object.assign({}, btnBase, { background: "linear-gradient(135deg, var(--purple), var(--pink))", border: "none", color: "#fff", fontWeight: 700 })}>
...
function Section(props) {
  return (
    <div style={{ marginBottom: 20 }}>
      <div style={{ fontSize: 11, color: "var(--text-secondary)", fontWeight: 800, textTransform: "uppercase", letterSpacing: "0.05em", marginBottom: 8 }}>{props.title}</div>
      {props.children}
    </div>
  );
}
function Stat(props) {
  return (
    <div style={{ background: "var(--bg-card-inner)", borderRadius: 8, padding: 14, border: "1px solid var(--border-light)" }}>
      <div style={{ fontSize: 9, color: "var(--text-secondary)", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.05em", marginBottom: 4 }}>{props.label}</div>
      <div style={{ fontSize: 20, fontWeight: 800, color: props.color || "var(--text-primary)" }}>{props.value}</div>
      {props.sub && <div style={{ fontSize: 10, color: "var(--text-muted)", marginTop: 2 }}>{props.sub}</div>}
    </div>
  );
}
function Table(props) {
  return (
    <div style={{ background: "var(--bg-card-inner)", borderRadius: 8, overflow: "hidden", border: "1px solid var(--border-light)" }}>
      <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
        <thead>
          <tr style={{ background: "var(--bg-page)" }}>
            {props.headers.map(function(h, i) {
              return <th key={i} style={{ padding: "8px 12px", textAlign: "left", fontSize: 10, color: "var(--text-secondary)", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.05em" }}>{h}</th>;
            })}
          </tr>
        </thead>
...
function Td(props) {
  return <td style={{ padding: "8px 12px", borderTop: "1px solid var(--border-light)", color: props.muted ? "var(--text-secondary)" : "var(--text-primary)" }}>{props.children}</td>;
}
function ScorePill(props) {
  if (props.score == null) return <span style={{ color: "var(--text-muted)" }}>{"\u2014"}</span>;
  var c = props.score >= 80 ? "var(--green)" : props.score >= 60 ? "var(--yellow)" : props.score >= 40 ? "var(--orange)" : "var(--red)";
  return <span style={{ color: c, fontWeight: 700 }}>{props.score}</span>;
}
function TicketLink(props) {
  return <a href={"https://cpr.repairq.io/ticket/" + props.num} target="_blank" rel="noopener noreferrer" style={{ color: "var(--cyan)", textDecoration: "none", fontWeight: 700 }}>#{props.num}</a>;
}
function Empty(props) {
  return <div style={{ padding: 16, color: "var(--text-muted)", fontSize: 12, textAlign: "center", background: "var(--bg-card-inner)", borderRadius: 6 }}>{props.children}</div>;
}
...
function Sparkline(props) {
  ...
  if (data.length >= 2 && lastVal != null) {
    ...
    if (priorVal != null) {
      ...
      lastValColor = improving ? "var(--green)" : "var(--red)";
    }
  }
  ...
  return (
    <div style={{ background: "var(--bg-card-inner)", borderRadius: 8, padding: "16px 12px", border: "1px solid var(--border-light)" }}>
      <div style={{ width: "100%", height: 110 }}>
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={chartData} margin={{ top: 5, right: 12, left: 0, bottom: 5 }}>
            <YAxis hide={true} />
            <XAxis dataKey="label" tick={{ fontSize: 10, fill: "#6B6F78" }} axisLine={{ stroke: "var(--border-light)" }} tickLine={false} />
            <Tooltip
              contentStyle={{ background: "var(--bg-page)", border: "1px solid #2A2D36", borderRadius: 6, fontSize: 11 }}
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: 6, paddingTop: 6, borderTop: "1px solid var(--border-light)", fontSize: 11, color: "var(--text-secondary)" }}>
...
function PeerCompare(props) {
  ...
  if (compareTarget != null && empVal != null) {
    ...
    empColor = equal ? "var(--yellow)" : (better ? "var(--green)" : "var(--red)");
  }
  return (
    <div style={{ background: "var(--bg-card-inner)", borderRadius: 8, padding: 14, border: "1px solid var(--border-light)" }}>
      <div style={{ fontSize: 10, color: "var(--text-muted)", marginBottom: 10 }}>{d.metric_label || "Comparison"}</div>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12 }}>
        <PeerCell label={props.employeeName} value={empVal} unit={unit} color={empColor} bold={true} />
        <PeerCell label={props.storeName + " avg"} value={storeVal} unit={unit} color="var(--text-secondary)" />
        <PeerCell label={(d.role_label || "All techs") + " avg"} value={roleVal} unit={unit} color="var(--text-secondary)" />
      </div>
      {compareTarget != null && empVal != null && (
        <div style={{ marginTop: 10, paddingTop: 10, borderTop: "1px solid var(--border-light)", fontSize: 11, color: "var(--text-secondary)", textAlign: "center" }}>
          {(function() {
            ...
            return better
              ? <span><strong style={{ color: "var(--green)" }}>{absDiff + unit} {lowerBetter ? "faster" : "above"}</strong> peer average</span>
              : <span><strong style={{ color: "var(--red)" }}>{absDiff + unit} {lowerBetter ? "slower" : "below"}</strong> peer average</span>;
          })()}
        </div>
      )}
    </div>
...
function PeerCell(props) {
  return (
    <div style={{ textAlign: "center", padding: "8px 4px" }}>
      <div style={{ fontSize: 9, color: "var(--text-muted)", textTransform: "uppercase", letterSpacing: "0.05em", marginBottom: 4, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{props.label}</div>
      <div style={{ fontSize: props.bold ? 22 : 18, fontWeight: props.bold ? 800 : 600, color: props.color || "var(--text-primary)" }}>
...
function DollarStake(props) {
  ...
  var tierColors = { "Bronze": "#CD7F32", "Silver": "#C0C0C0", "Gold": "var(--yellow)", "Platinum": "var(--cyan)", "Diamond": "var(--pink)" };
  var fromColor = tierColors[s.current_tier] || "var(--text-secondary)";
  var toColor = tierColors[s.projected_tier] || "var(--text-secondary)";
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 16, flexWrap: "wrap" }}>
        <div style={{ flex: 1, minWidth: 200 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 8 }}>
            ...
            <span style={{ fontSize: 11, color: "var(--text-muted)" }}>({s.current_overall} {"\u2192"} {s.projected_overall} pts)</span>
          </div>
          {s.lift_source && (
            <div style={{ fontSize: 10, color: "var(--text-muted)", fontStyle: "italic" }}>{"\u2014"} based on {s.lift_source}</div>
          )}
        </div>
        <div style={{ textAlign: "center" }}>
          <div style={{ fontSize: 9, color: "var(--text-muted)", textTransform: "uppercase", letterSpacing: "0.05em", marginBottom: 4 }}>Per Month</div>
          <div style={{ fontSize: 26, fontWeight: 800, color: delta > 0 ? "var(--green)" : "var(--text-secondary)" }}>
            {delta > 0 ? "+$" + delta : "$0"}
          </div>
          <div style={{ fontSize: 10, color: "var(--text-muted)", marginTop: 2 }}>commission impact</div>
        </div>
      </div>
      {s.diamond_bonus_added && (
        <div style={{ marginTop: 10, padding: "6px 10px", background: "#FF2D9511", borderLeft: "3px solid var(--pink)", borderRadius: 4, fontSize: 11, color: "var(--pink)", fontWeight: 700 }}>
