...
export default function VoicemailTab({ storeFilter }) {
  ...
  var storeChartData = useMemo(function() {
    ...
    return STORE_KEYS.map(function(sk) {
      ...
      var store = STORES[sk];
      return { name: store ? store.name.replace("CPR ", "") : sk, returned: s.returned || 0, unreturned: s.unreturned || 0, rate: s.rate || 0, color: store ? store.color : "var(--text-secondary)" };
  ...
  if (loading) return <div style={{ padding:40,textAlign:"center",color:"var(--text-muted)" }}>Loading voicemail data...</div>;
      ...
      <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20,flexWrap:"wrap",gap:8 }}>
        ...
        <div style={{ display:"flex",gap:4 }}>
          {[{k:"all",l:"All"},{k:"urgent",l:"Urgent"},{k:"unreturned",l:"Unreturned"},{k:"returned",l:"Returned"}].map(function(f) {
            return <button key={f.k} onClick={function(){setStatusFilter(f.k);}} style={{ padding:"5px 12px",borderRadius:6,border:"none",cursor:"pointer",background:statusFilter===f.k?"var(--border)":"transparent",color:statusFilter===f.k?"var(--text-primary)":"var(--text-muted)",fontSize:11,fontWeight:600 }}>{f.l}{f.k==="urgent"&&summary&&summary.urgent>0?" ("+summary.urgent+")":""}</button>;
          })}
        </div>
      </div>
      ...
      {view === "dashboard" && summary && (
        <div>
          <div style={{ display:"grid",gridTemplateColumns:"repeat(5,1fr)",gap:14,marginBottom:28 }}>
            <StatCard label="Total Voicemails" value={summary.total} accent="var(--purple)" sub="last 30 days" />
            <StatCard label="Return Rate" value={returnRate+"%"} accent={parseFloat(returnRate)>=80?"var(--green)":parseFloat(returnRate)>=60?"var(--yellow)":"var(--red)"} sub={summary.returned+" of "+summary.total+" returned"} />
            <StatCard label="Unreturned" value={summary.unreturned} accent="var(--red)" sub={summary.urgent+" urgent (1hr+)"} />
            <StatCard label="Avg Callback" value={summary.avg_callback_min > 0 ? callbackTime(summary.avg_callback_min) : "—"} accent="var(--cyan)" sub="time to return" />
            <StatCard label="Urgent" value={summary.urgent} accent={summary.urgent>0?"var(--red)":"var(--green)"} sub="unreturned > 1 hour" />
          </div>
          ...
          <div style={{ display:"grid",gridTemplateColumns:"repeat("+Math.min(STORE_KEYS.length,3)+",1fr)",gap:14,marginBottom:20 }}>
            {STORE_KEYS.map(function(sk) {
              ...
              var sc = parseFloat(rate) >= 80 ? "var(--green)" : parseFloat(rate) >= 60 ? "var(--yellow)" : "var(--red)";
              return (
                <div key={sk} style={{ background:"var(--bg-card)",borderRadius:12,padding:20,border:"1px solid "+store.color+"33" }}>
                  <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14 }}>
                    <div style={{ display:"flex",alignItems:"center",gap:8 }}>
                      ...
                      <div>
                        ...
                        <div style={{ color:"var(--text-muted)",fontSize:11 }}>{s.total || 0} voicemails</div>
                      </div>
                    </div>
                  ...
                  <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8 }}>
                    <div style={{ background:"var(--bg-card-inner)",borderRadius:8,padding:"8px 10px",textAlign:"center" }}>
                      <div style={{ color:"var(--text-secondary)",fontSize:9,textTransform:"uppercase" }}>Returned</div>
                      <div style={{ color:"var(--green)",fontSize:16,fontWeight:700 }}>{s.returned || 0}</div>
                    </div>
                    <div style={{ background:"var(--bg-card-inner)",borderRadius:8,padding:"8px 10px",textAlign:"center" }}>
                      <div style={{ color:"var(--text-secondary)",fontSize:9,textTransform:"uppercase" }}>Unreturned</div>
                      <div style={{ color:"var(--red)",fontSize:16,fontWeight:700 }}>{s.unreturned || 0}</div>
                    </div>
                    <div style={{ background:"var(--bg-card-inner)",borderRadius:8,padding:"8px 10px",textAlign:"center" }}>
                      <div style={{ color:"var(--text-secondary)",fontSize:9,textTransform:"uppercase" }}>Urgent</div>
                      <div style={{ color:s.urgent > 0 ? "var(--red)" : "var(--green)",fontSize:16,fontWeight:700 }}>{s.urgent || 0}</div>
                    </div>
                  </div>
                  <div style={{ marginTop:12,background:"var(--bg-card-inner)",borderRadius:6,height:8,overflow:"hidden" }}>
          ...
          {/* Urgent voicemails */}
          {summary.urgent > 0 && (
            <div style={{ background:"var(--bg-card)",borderRadius:12,padding:20,marginBottom:20,border:"1px solid #F8717133" }}>
              ...
              <div style={{ maxHeight:300,overflowY:"auto" }}>
                {voicemails.filter(function(v) { return v.urgent; }).map(function(vm, i) {
                  ...
                  return (
                    <div key={vm.call_id || i} style={{ padding:"12px 0",borderBottom:"1px solid var(--border)",display:"flex",justifyContent:"space-between",alignItems:"center" }}>
                      <div style={{ display:"flex",alignItems:"center",gap:12 }}>
                        <div style={{ width:36,height:36,borderRadius:8,background:"#F8717122",display:"flex",alignItems:"center",justifyContent:"center",color:"var(--red)",fontSize:16 }}>{"\uD83D\uDCF1"}</div>
                        <div>
                          <div style={{ color:"var(--text-primary)",fontSize:14,fontWeight:700 }}>{formatPhone(vm.phone)}</div>
                          <div style={{ display:"flex",gap:8,alignItems:"center" }}>
                            <span style={{ color:store?store.color:"var(--text-secondary)",fontSize:11 }}>{store?store.name.replace("CPR ",""):vm.store}</span>
                            <span style={{ color:"var(--text-muted)",fontSize:11 }}>{d.toLocaleDateString()+" "+d.toLocaleTimeString([],{hour:"2-digit",minute:"2-digit"})}</span>
                          </div>
                        </div>
                      </div>
                      <div style={{ display:"flex",alignItems:"center",gap:10 }}>
                        <span style={{ color:"var(--red)",fontSize:12,fontWeight:700 }}>{timeAgo(vm.age_minutes)}</span>
                        <a href={"tel:"+vm.phone} style={{ padding:"6px 14px",borderRadius:6,background:"#4ADE8022",color:"var(--green)",fontSize:11,fontWeight:700,textDecoration:"none",cursor:"pointer" }}>Call Back</a>
                      </div>
                    </div>
                  );
                })}
              ...
              <div style={{ height:200 }}>
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={storeChartData}>
                    ...
                    <Tooltip contentStyle={{background:"var(--border-light)",border:"1px solid var(--border)",borderRadius:8}} formatter={function(v){return v+"%";}} />
                    <Bar dataKey="rate" name="Return Rate" radius={[6,6,0,0]} barSize={40}>
                      {storeChartData.map(function(entry, i) {
                        var c = entry.rate >= 80 ? "var(--green)" : entry.rate >= 60 ? "var(--yellow)" : "var(--red)";
      ...
      {view === "list" && (
        <div>
          ...
          <div style={{ background:"var(--bg-card)",borderRadius:12,padding:20,maxHeight:700,overflowY:"auto" }}>
            {/* Header */}
            <div style={{ display:"grid",gridTemplateColumns:"2fr 1fr 1fr 1.5fr 80px",gap:8,padding:"0 0 10px",borderBottom:"1px solid var(--border)" }}>
              {["Caller","Store","Left At","Callback","Status"].map(function(h,i) {
                return <div key={i} style={{ color:"var(--text-muted)",fontSize:10,textTransform:"uppercase",fontWeight:600 }}>{h}</div>;
              })}
            </div>
            ...
            {filtered.map(function(vm, i) {
              ...
              var d = new Date(vm.date);
              var statusConfig = {
                unreturned: { label: "Unreturned", color: "var(--red)", bg: "#F8717112" },
                returned_fast: { label: "< 30 min", color: "var(--green)", bg: "#4ADE8012" },
                returned_ok: { label: "< 1 hr", color: "var(--yellow)", bg: "#FBBF2412" },
                returned_late: { label: "> 1 hr", color: "var(--orange)", bg: "#FB923C12" },
              };
              var sc = statusConfig[vm.status] || statusConfig.unreturned;

              return (
                <div key={vm.call_id || i} style={{ display:"grid",gridTemplateColumns:"2fr 1fr 1fr 1.5fr 80px",gap:8,padding:"12px 0",borderBottom:"1px solid var(--border-light)",alignItems:"center",opacity:vm.status==="unreturned"?1:0.85 }}>
                  <div>
                    <div style={{ color:"var(--text-primary)",fontSize:13,fontWeight:600 }}>{formatPhone(vm.phone)}</div>
                    {vm.urgent && <span style={{ color:"var(--red)",fontSize:9,fontWeight:700 }}>URGENT — {timeAgo(vm.age_minutes)}</span>}
                  </div>
                  <div>
                    <span style={{ display:"inline-flex",alignItems:"center",gap:4,fontSize:11,color:store?store.color:"var(--text-secondary)" }}>
                      <span style={{ width:6,height:6,borderRadius:"50%",background:store?store.color:"var(--text-secondary)" }} />
                  ...
                  <div>
                    {vm.callback_date ? (
                      ...
                      <div style={{ display:"flex",alignItems:"center",gap:6 }}>
                        <span style={{ color:"var(--red)",fontSize:11 }}>Not returned</span>
                        <a href={"tel:"+vm.phone} style={{ padding:"3px 8px",borderRadius:4,background:"#4ADE8018",color:"var(--green)",fontSize:10,fontWeight:600,textDecoration:"none" }}>Call</a>
                      </div>
                    )}
                  </div>
                  <div>
