...
export default function CallPerformanceTab({ storeFilter, overviewStats, dailyCalls, hourlyMissed, dowData, callbackData, problemCalls }) {
  ...
  var filteredStats = useMemo(function() {
    var storeStats = {};
    STORE_KEYS.forEach(function(sk) { storeStats[sk] = { answered: 0, missed: 0, total: 0, afterHoursMissed: 0 }; });
    var totalAnswered = 0, totalMissed = 0, totalAfterHours = 0;
    ...
    filteredDailyCalls.forEach(function(d) {
      STORE_KEYS.forEach(function(sk) {
        ...
        var a = d[sk + "_answered"] || 0;
        var m = d[sk + "_missed"] || Math.max(0, t - a);
        var ah = d[sk + "_after_hours_missed"] || 0;
        storeStats[sk].answered += a;
        storeStats[sk].missed += m;
        storeStats[sk].total += t;
        storeStats[sk].afterHoursMissed += ah;
        totalAnswered += a;
        totalMissed += m;
        totalAfterHours += ah;
      });
    ...
    return {
      totals: { answered: totalAnswered, missed: totalMissed, afterHoursMissed: totalAfterHours, avgDuration: overviewStats?.totals?.avgDuration || 0, vmRate: overviewStats?.totals?.vmRate || 0 },
      storeStats: storeStats,
    };
  }, [filteredDailyCalls, overviewStats]);
        ...
        <div>
          ...
          <div style={{ display:"grid",gridTemplateColumns:"repeat("+STORE_KEYS.length+",1fr)",gap:14,marginBottom:20 }}>
            {STORE_KEYS.map(function(sk) {
              ...
              var cbRate = cb && cb.missed > 0 ? Math.round(((cb.calledBack || cb.within30 + cb.within60 + cb.later || 0) / cb.missed) * 100) : 0;
              var storeMissedRev = Math.round(s.missed * CONV_RATE * AVG_TICKET);
              // After-hours (closed) calls: excluded from answer rate. Show the split.
              var afterHours = s.afterHoursMissed || 0;
